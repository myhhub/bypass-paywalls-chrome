    ...
    if (enabledSites.includes('#options_enable_new_sites')) {
      ...
      ext_api.management.getSelf(function (result) {
        if ((result.installType === 'development' || (result.installType !== 'development' && !enabledSites.includes('#options_on_update')))) {
          let new_groups = ['###_uk_independent'];
...
ext_api.runtime.onMessage.addListener(function (message, sender) {
  ...
  if (message.request === 'custom_domain' && message.data && message.data.domain) {
    ...
    if (group) {
      let nofix_groups = ['###_au_nomedia', '###_beehiiv', '###_fi_alma_talent', '###_fi_kaleva', '###_ghost', '###_it_citynews', '###_nl_vmnmedia', '###_se_gota_media', '###_substack_custom', '###_uk_aspermont', '###_usa_cherryroad'];
  ...
  if (message.request === 'getExtFetch' && message.data) {
    message.data.html = '';
    fetch(message.data.url, message.data.headers)
    .then(response => {
      if (response.ok) {
