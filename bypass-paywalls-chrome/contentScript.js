...
if (matchDomain("heise.de"))
  dompurify_options.ADD_TAGS = dompurify_options.ADD_TAGS.concat([
    "a-gift",
    "a-img",
  ]);

var ar_grupo_clarin_domains = [
  "clarin.com",
  "lavoz.com.ar",
  "losandes.com.ar",
  "ole.com.ar",
...
var be_mediahuis_domains = [
  "gva.be",
  "hbvl.be",
  "nieuwsblad.be",
  "standaard.be",
];
var be_roularta_domains = [
  "artsenkrant.com",
...
var ca_gcm_domains = ["lesoleil.com"].concat([
  "latribune.ca",
  "lavoixdelest.ca",
  "ledroit.com",
  "ledroitfranco.com",
...
var es_epiberica_custom_domains = [
  "diaridegirona.cat",
  "diariocordoba.com",
  "diariodeibiza.es",
  "elcorreogallego.es",
...
var fr_groupe_la_depeche_domains = [
...
var fr_groupe_nice_matin_domains = [
  "monacomatin.mc",
  "nicematin.com",
  "varmatin.com",
];
var it_ilmessaggero_domains = [
...
var ke_nation_media_custom_domains = [
  "monitor.co.ug",
  "mwananchi.co.tz",
  "mwanaspoti.co.tz",
  "thecitizen.co.tz",
...
var pe_grupo_elcomercio_domains = [
  "diariocorreo.pe",
  "elcomercio.pe",
  "gestion.pe",
];
var pl_ringier_domains = [
  "auto-swiat.pl",
...
var usa_arizent_custom_domains = [
  "accountingtoday.com",
  "benefitnews.com",
  "bondbuyer.com",
  "dig-in.com",
...
function clearLocalStorage(bg2csData = "") {
  ...
  if (
    bg2csData &&
    bg2csData.cs_clear_lclstrg &&
    !matchDomain(
      ["britannica.com", "nationalreview.com", "thecritic.co.uk"].concat(
        usa_mcc_domains
      )
    )
  )
...
function run_custom(bg2csData, dompurify_loaded) {
        ...
        if (paywall.length && article) {
          ...
          if (json_script) {
            try {
              ...
              let json_key =
                findKeyJson(json, /^articlebody$/i) ||
          ...
          if (json_script) {
            try {
              ...
              if (url_next && !window.location.pathname.endsWith(url_next))
                refreshCurrentTab();
              let json_text = findKeyJson(
                json,
                ["blocks", "body", "content", "contentHtml", "description"],
                500
              );
    ...
    if (data.includes("|")) {
      window.setTimeout(function () {
        ...
        let filter = new RegExp(
          data_split[2].replace(/\./g, "\\.").replace("=", "\\s?=\\s?")
        );
  ...
  if (bg2csData.ld_json_url && dompurify_loaded) {
    ...
    if (data.includes("|")) {
      window.setTimeout(function () {
        ...
        let article_id;
        if (article_id_sel) {
          let article_id_sel_dom = document.querySelector(
            article_id_sel + "[content]"
          );
          if (article_id_sel_dom) article_id = article_id_sel_dom.content;
          else return;
        }
        function setMediaSrc(elem) {
          if (elem.getAttribute("data-src"))
            ...
            let data_src = [...elem.attributes].find((x) =>
              x.name.endsWith("-src")
            );
        ...
        func_post = function () {
          ...
          let hidden_images = document.querySelectorAll(
            "figure " + img_sel + ", picture " + img_sel
          );
          for (let elem of hidden_images) setMediaSrc(elem);
          let iframes = document.querySelectorAll('iframe[src="about:blank"]');
          for (let elem of iframes) setMediaSrc(elem);
        };
        getJsonUrl(
          paywall_sel,
          "",
          article_sel,
          {
            art_append: article_append,
            art_hold: article_hold,
  ...
  if (bg2csData.ld_archive_is && dompurify_loaded) {
    ...
    if (data.includes("|")) {
      window.setTimeout(function () {
        ...
        func_post = function () {
          if (mobile) {
            let lazy_images = document.querySelectorAll(
  ...
  if (bg2csData.amp_redirect) {
    window.setTimeout(function () {
      let amp_script = document.querySelector(
  ...
  function cs_code_elems(elems) {
    for (let elem of elems) {
      ...
      for (let item of elem_dom) {
        if (elem.rm_elem) removeDOMElement(item);
        if (elem.rm_class) {
        ...
        if (elem.rm_attrib) {
          ...
          for (let rm_attrib of rm_attribs) item.removeAttribute(rm_attrib);
        }
        ...
        if (elem.elems) cs_code_elems(elem.elems);
      }
      if (elem.rm_elem_wait)
        waitDOMElement(
          elem.cond,
          elem.cond.match(/^\w+/)[0].toUpperCase(),
          removeDOMElement,
          true
        );
    }
...
function run_cs_default(bg2csData = "") {
  for (let n = 0; n < 5; n++) {
    setTimeout(function () {
      ...
      if (!bg2csData.cs_block) cs_default(bg2csData);
    }, n * 200);
  }
}
...
var url_old;
if (ext_api.runtime) {
  ext_api.runtime.onMessage.addListener(function (request, sender) {
    if (request.msg === "bg2cs") {
      ...
      if (!(msg_once && url_old === window.location.href)) {
        ...
        if (
          Object.keys(bg2csData).filter(
            (x) => ![("cs_block", "cs_clear_lclstrg")].includes(x)
          ).length
        )
      ...
      if (!(msg_once_ses && url_old === window.location.href)) {
        ...
        replaceDomElementExtSrc(
          request.data.url,
          request.data.url_src,
          request.data.html,
          true,
          false,
          request.data.selector,
          request.data.text_fail,
          request.data.selector_source,
          request.data.selector_archive
        );
      }
    } else if (request.msg === "showExtFetch") {
      let fetch_id = request.data.data_ext_fetch_id;
      if (data_ext_fetch[fetch_id]) {
        data_ext_fetch[fetch_id].func(
          request.data.url,
          request.data.html,
          ...data_ext_fetch[fetch_id].args
        );
      }
    }
  });
      ...
      if (event.data) {
        if (event.data.type === "from_page") {
          ...
          if (data && article && dompurify_loaded && !msg_once_page) {
            ...
            let parser = new DOMParser();
            let doc = parser.parseFromString(
              "<div>" + DOMPurify.sanitize(data, dompurify_options) + "</div>",
              "text/html"
            );
            let article_new = doc.querySelector("div");
            if (article_new) {
              if (matchDomain("businesspost.ie")) {
                let iframes = article_new.querySelectorAll(
                  "div.article__iframe > iframe[height]"
                );
                for (let elem of iframes)
                  elem.style =
                    "height: " + (Number(elem.height) + 25) + "px; !important";
              }
              article.innerHTML = "";
              article.appendChild(article_new);
            }
...
function cs_default(bg2csData = "") {
  if (bg2csData && bg2csData.cs_param) cs_param = bg2csData.cs_param;

  var overlay = document.querySelector("body.didomi-popup-open");
  if (overlay) overlay.classList.remove("didomi-popup-open");
  var ads =
    'div.OUTBRAIN, div[id^="taboola-"], div.ad-container, div[class*="-ad-container"], div[class*="_ad-container"], div.arc_ad, div[id^="poool-"], amp-ad, amp-embed[type="mgid"], amp-embed[type="outbrain"], amp-embed[type="taboola"]';
  hideDOMStyle(ads, 10);

  if (!(csDone || csDoneOnce)) {
    if (
      matchDomain("medium.com") ||
      matchDomain(medium_custom_domains) ||
      ...
      window.location.hostname.match(/\.(au|nz)$/) ||
      matchDomain(["afr.com", "inc-aus.com"])
    ) {
      ...
      if (matchDomain("afr.com")) {
        ...
        if (article) {
          window.setTimeout(function () {
            let pars = article.querySelectorAll(
              "p:not([class]), figure:not(:empty)"
            );
            let pagination = document.querySelector(
              "div > span#pagination-top"
            );
            if (
              ((pars.length && pars.length < 5) || pagination) &&
              dompurify_loaded
            ) {
              if (pagination) {
                        ...
                        try {
                          let json = JSON.parse(
                            html
                              ...
                              .split("};")[0]
                              .replace(/:undefined([,}])/g, ':"undefined"$1') +
                              "}"
                          );
                          if (json) {
                            let placeholders;
                            function find_item(match, p1, offset, string) {
                              ...
                              if (
                                ...
                                if (item.data) {
                                  if (
                                    ["linkArticle", "linkExternal"].includes(
                                      item.type
                                    )
                                  ) {
                                    if (item.data.text) {
                                    ...
                                    ["instagram", "twitter"].includes(item.type)
                                  ) {
                                    if (item.data.url)
                                      result =
                                        '<a href="' +
                            ...
                            if (json_text) {
                              placeholders =
                                json.page.content.asset.bodyPlaceholders;
                              if (placeholders)
                              ...
                              addStyle(
                                "section:not([class]) > p {margin: 24px 0px;}"
                              );
                              let key_posts = document.querySelectorAll(
                                'li > a[href*="?post="]'
                              );
          ...
          fetch(url).then((response) => {
            if (response.ok) {
              response.text().then((html) => {
                ...
                if (match) {
                  let parser = new DOMParser();
                  let src_text = breakText(parseHtmlEntities(match[1]))
                    .replace(/\n\n/g, "<br><br>")
                    .replace(/\.([^\s\d]|&)/g, ". $1");
                  let doc = parser.parseFromString(
                    "<div>" +
        ...
        if (matchDomain("themandarin.com.au")) {
          getJsonUrl(
            'div[data-enterprise-agreement-paywall="true"]',
            "",
            "div.paywall-mandy"
          );
        } else {
        ...
        let fade = document.querySelector(
        ...
        if (paywall[0] && dompurify_loaded) {
          ...
          if (json_script) {
            try {
              let json = JSON.parse(json_script.text);
              let json_text = json.filter(
                (x) => typeof x === "string" && x.match(/(<|\\u003C)p>/)
              )[0];
              let parser = new DOMParser();
              let doc = parser.parseFromString(
                "<div>" + DOMPurify.sanitize(json_text) + "</div>",
                "text/html"
              );
              let content_new = doc.querySelector("div");
        ...
        getJsonUrl("section.paywall", "", "div.article-body", {
          art_append: 1,
        });
        ...
        if (!window.location.hostname.startsWith("amp.")) {
          amp_redirect(
            'head > meta[content^="FOR SUBSCRIBERS"], #paywall_prompt'
          );
        } else {
          amp_unhide_subscr_section();
        }
        ...
        let au_comm_media_link = document.querySelector(
          'a[href^="https://austcommunitymedia.my.site.com/"]'
        );
        if (matchDomain(au_comm_media_domains) || au_comm_media_link) {
          ...
          if (mask) {
            ...
            let div_hidden = document.querySelectorAll(
              "div.flex-col div.hidden"
            );
            for (let elem of div_hidden) elem.classList.remove("hidden");
          } else {
            let subscribe_truncate = document.querySelector(
              ".subscribe-truncate"
            );
            if (subscribe_truncate)
              subscribe_truncate.classList.remove("subscribe-truncate");
            let subscriber_hiders =
              document.querySelectorAll(".subscriber-hider");
          ...
          let ads =
            '.ad-placeholder, .sticky, [id*="-container"], #hindsight-ads-iframe';
          hideDOMStyle(
            story_generic_iframe + ", " + blocker + ", " + overlays + ", " + ads
          );
        } else if (window.location.hostname.endsWith(".com.au")) {
          // Australia News Corp
          let au_news_corp_domains = [
            "adelaidenow.com.au",
            "cairnspost.com.au",
            "codesports.com.au",
            "couriermail.com.au",
          ...
          if (matchDomain(au_news_corp_domains)) {
            ...
            if (url.includes("/subscribe/")) {
              if (
                ...
                url.includes("dest=") &&
                url.split("dest=")[1].split("&")[0]
              ) {
                let url_new =
                  decodeURIComponent(url.split("dest=")[1].split("&")[0]) +
                  "?amp";
                window.setTimeout(function () {
            ...
            if (
              ...
              document.querySelector(
                'head > link[href="https://images.thewest.com.au"]'
              )
            ) {
                ...
                if (json_script) {
                  ...
                  try {
                    ...
                    for (let key in json_article) {
                      let json_resolution =
                        json_article[key].data.result.resolution;
                    ...
                    for (let par of json_content) {
                      ...
                      if (par.kind === "text") {
                        ...
                        par_elem.innerText =
                          (par.attribution ? par.attribution + ": " : "") +
                          par.text;
                    ...
                    let content = document.querySelector(
                      'div[class*="StyledArticleContent"]'
                    );
                    if (content) {
                      content.innerHTML = "";
              ...
              if (paywall) {
                removeDOMElement(paywall);
                let lockable = document.querySelectorAll(
                  'div[id^="lockable-"]'
                );
      ...
      window.location.hostname.match(/\.(de|at|ch)$/) ||
      matchDomain(["faz.net", "handelsblatt.com", "tt.com", "wochenblatt.com"])
    ) {
      //germany/austria/switzerland - ch
      ...
      if (matchDomain("aachener-zeitung.de")) {
        ...
        if (shade) shade.classList.remove("paywalled-article");
        let noscroll = document.querySelectorAll("html[class], body[class]");
        for (let elem of noscroll) elem.removeAttribute("class");
        ...
        if (paywall && dompurify_loaded) {
          ...
          let og_url = document.querySelector(
            'head > meta[name="og:url"][content]'
          );
          if (og_url && !og_url.content.endsWith(url_id)) refreshCurrentTab();
          let json;
          if (html.includes("window.__NUXT__="))
            json = html
              .split("window.__NUXT__=")[1]
              .split("</script>")[0]
              .trim()
              .replace(/blocs:\[\{.*?\}\],/g, "");
          ...
          if (article && json) {
            ...
            if (content) {
              ...
              let iframely = content_new.querySelectorAll(
                "div > div.fr-iframely"
              );
        ...
        if (body_hidden) body_hidden.removeAttribute("class");
        let lazy_images = document.querySelectorAll("img.lazy[data-src]");
        for (let elem of lazy_images) {
          elem.src = elem.getAttribute("data-src");
          elem.removeAttribute("class");
        }
        let lazy_sources = document.querySelectorAll(
        ...
        if (paywall && dompurify_loaded) {
          ...
          if (json_script) {
            try {
              let json = JSON.parse(json_script.text);
              if (json) {
                let url_id = json_script.text.includes('"gcid":"')
                  ? json_script.text.split('"gcid":"')[1].split('"')[0]
                  : "";
                ...
                if (article) {
                  ...
                  for (let paragraph of paragraphs) paragraph.innerHTML = "";
                  let parser = new DOMParser();
                  for (let par in pars) {
                    ...
                    let sub_elem = "";
                    if (
                      par_elem.__typename === "TextParagraph" &&
                      par_elem.text
                    ) {
                      let content_new = parser.parseFromString(
                        "<div>" + DOMPurify.sanitize(par_elem.text) + "</div>",
                        "text/html"
                      );
                      sub_elem = content_new.querySelector("div");
                    } else if (
                      par_elem.__typename === "EmbedParagraph" &&
                      par_elem.embedCode
                    ) {
                      let content_new = parser.parseFromString(
                        "<div>" +
                      ...
                      if (par_elem.credit) {
                        ...
                        sub_elem.appendChild(
                          document.createTextNode(par_elem.credit)
                        );
                      }
        ...
        let ads =
          'div[class^="traffective_"], div[class^="article_billboard-"], div[class*="_ad_"], div[class^="outbrain_"]';
        hideDOMStyle(ads);
      } else if (matchDomain("bild.de")) {
        func_post = function () {
          if (mobile) {
            let lazy_images = document.querySelectorAll(
              'figure img[loading="lazy"][style]'
            );
            for (let elem of lazy_images) {
        ...
        if (!window.location.search.match(/(\?|&)amp/)) {
          ...
          if (paywall && dompurify_loaded) {
            removeDOMElement(paywall);
            func_post = function () {
              let related_images = document.querySelectorAll(
                "amp-carousel a > amp-img[src]"
              );
              ...
              let img_captions = document.querySelectorAll(
          ...
          let teasered_content_fader = document.querySelector(
            ".teasered-content-fader"
          );
        ...
        if (matchDomain("zeitung.faz.net")) {
          ...
          if (paywall_z) {
            ...
            if (og_url) window.location.href = og_url.content;
            else header_nofix("div.article__text");
          }
        ...
        if (paywall) {
          removeDOMElement(paywall);
          let related = document.querySelector(
            "div.c-teaser-plus-related--paywall"
          );
          if (related)
              ...
              if (hidden_article) {
                ...
                for (let par of pars) {
                  ...
                  if (par_first) {
                    ...
                    if (overlap)
                      intro_last.innerText =
                        intro_last.innerText.replace(
                          new RegExp(overlap + "$"),
                          ""
                        ) + par;
                    par_first = false;
        ...
        let related_topics = document.querySelector(
          "app-storyline-related-topics"
        );
        ...
        let charts = document.querySelectorAll(
          'iframe[name^="iframe-"][loading][src*="grafik.handelsblatt.com"]'
        );
        ...
        let paywall = pageContains(
          "h3.m-auto",
          "Dieser Artikel ist nur mit Abo zugänglich."
        );
        if (paywall.length) {
          ...
          if (json_script) {
            try {
              ...
              if (
                json &&
                json.props.pageProps.sections &&
                json.props.pageProps.sections[1].content
              ) {
                ...
                let first_par = document.querySelector(
                  "body > div#__next p.bodyText"
                );
                if (first_par) {
                  ...
                  if (article) {
                    ...
                    for (let par of pars) {
                      if (!add_par) {
                        if (par.type === "paywall") add_par = true;
                      } else {
                        if (par.text) {
                          let elem_type = "p";
                          ...
                          if (["paragraph", "quote"].includes(par.type)) {
                            ...
                            elem_type = "h2";
                            elem_class =
                              "content-element font-headline h2 my-1em";
                          }
                          ...
                          article.appendChild(
                            content_new.querySelector(elem_type)
                          );
        ...
        function unhide_article(node) {
          ...
          let article = document.querySelector(
            "div[data-article-content][style]"
          );
        ...
        if (window.location.pathname.endsWith("/amp/")) ampToHtml();
        ...
        if (paywall) {
          ...
          if (json_script) {
            ...
            if (json) {
              ...
              let article = paywall.querySelector(
        ...
        for (let elem of fade) elem.classList.remove("nzzinteraction");
        let ads = "div.resor";
        hideDOMStyle(ads);
        ...
        getJsonUrl(
          "div.entry-paywall-login",
          "",
          "div.entry-main > div.entry__post-content"
        );
      } else if (matchDomain("spektrum.de")) {
        let paywall = document.querySelector("article.pw-premium");
        if (paywall) paywall.classList.remove("pw-premium");
      } else if (matchDomain(["spiegel.de", "manager-magazin.de"])) {
        let url = window.location.href;
        func_post = function () {
          let failed_iframes = document.querySelectorAll(
            'div > div[x-show="!iframeIsLoaded"]'
          );
          for (let elem of failed_iframes) hideDOMElement(elem.parentNode);
          let body_dark = document.querySelector('body[class*="dark:"]');
          if (body_dark) removeClassesByPrefix(body_dark, "dark:");
          let charts = document.querySelectorAll(
            'section div[x-data*="{isLoaded:"]'
          );
          for (let elem of charts) elem.style.height = elem.offsetHeight + "px";
          if (mobile) {
            let lazy_images = document.querySelectorAll(
        ...
        if (matchDomain("sz-magazin.sueddeutsche.de")) {
          getArchive(
            url,
            "div.articlemain__inner--reduced",
            { rm_class: "articlemain__inner--reduced" },
            "main"
          );
        } else if (window.location.pathname.startsWith("/projekte/artikel/")) {
          func_post = function () {
            let lazy_images = document.querySelectorAll(
              'img[loading="lazy"][style*="min-width:"]'
            );
            for (let elem of lazy_images)
              elem.style = "width: 80%; margin: auto;";
            let sticky = document.querySelectorAll(
              'div > div > div[old-position="sticky"]'
            );
            for (let elem of sticky) {
              let div_hidden = elem.parentNode.parentNode.querySelector(
          ...
          if (paywall && dompurify_loaded) {
            ...
            let article = document.querySelector(article_sel);
            if (article) {
              let json_script = document.querySelector(
                'script[data-hydration-props-component-name="ArticleBodyDDRum"]'
              );
              if (json_script) {
                try {
                  ...
                  if (json) {
                    ...
                    for (let par of pars) {
                      ...
                      if (
                        ["paragraph", "datawrapper", "youtube"].includes(
                          par.component
                        )
                      ) {
                        if (par.content && par.content.html) {
                          let elem_type = par.content.html.startsWith("<div>")
                            ? "div"
                            : "p";
                          let content_new = parser.parseFromString(
                            "<" +
                              ...
                              ">" +
                              DOMPurify.sanitize(
                                parseHtmlEntities(par.content.html),
                                dompurify_options
                              ) +
                              "</" +
                              elem_type +
                          ...
                          if (iframe)
                            iframe.style =
                              "width: 100%; height: 400px; margin-bottom: 32px;";
                        ...
                        if (par.content && par.content.image) {
                          let caption = par.content.caption
                            ? par.content.caption.html +
                              " (Foto: " +
                              par.content.imageSource +
                              ")"
                            : "";
                          let sub_elem = makeFigure(
                            par.content.image.url,
                            caption
                          );
                          elem.appendChild(sub_elem);
                        }
                        ...
                        !(
                          [
                            ...
                            "articleTeaserM",
                            "newsletterEmbed",
                          ].includes(par.component) ||
                          par.component.startsWith("iqadtile")
                        )
                      )
                        console.log(par);
        ...
        if (matchDomain("www.tagesspiegel.de")) {
          func_post = function () {
            let opinionary = document.querySelector(
              "div > div#opinary-automation-placeholder"
            );
            if (opinionary) hideDOMElement(opinionary.parentNode);
            if (mobile) {
              let lazy_images = document.querySelectorAll(
                'figure img[loading="lazy"][style]'
              );
        ...
        window.setTimeout(function () {
          ...
          if (paywall && dompurify_loaded) {
            ...
            if (article) {
              let json_script = document.querySelector(
                "script#tt-com-www-state"
              );
              if (json_script) {
                try {
                  let json_articles = JSON.parse(json_script.text)
                    .TT_COM_WWW_GLOBAL_STATE.articles;
                  let json_article_id = json_articles.ids[0];
                  if (
                    !json_article_id ||
                    (json_article_id &&
                      !window.location.pathname.includes(json_article_id))
                  )
                    refreshCurrentTab();
                  let parser = new DOMParser();
                  let pars =
                    json_articles.entities[json_article_id].articleData.article
                      .elements;
                  for (let par of pars) {
                    let elem;
              ...
              if (!paywall.hasChildNodes()) {
                ...
                if (json_script) {
                  try {
                    ...
                    let article_new = doc.querySelector("div");
                    let hidden_images = article_new.querySelectorAll(
                      'img[src^="/"][srcset]'
                    );
                    let json_domain =
                      json.content.data.post.thumbnail.src.match(
                        /https:\/\/(www\.)?\w+\.at/
                      )[0];
                    ...
                    let hidden_comments = document.querySelector(
        ...
        func_post = function () {
          if (mobile) {
            let headers = document.querySelectorAll(
              "main header, main header ~ div"
            );
            for (let elem of headers) elem.removeAttribute("style");
            let main_divs = document.querySelectorAll(
              "main div[style] > div > div[id]"
            );
            ...
            let lazy_images = document.querySelectorAll(
              'main img[loading="lazy"][style]'
            );
            for (let elem of lazy_images) elem.style = "width: 95%;";
          }
          header_nofix(
            "main header",
            'img[alt="WELTplus"][loading]',
            "BPC > no archive-fix"
          );
        ...
        getArchive(
          url,
          "div.contains_walled_content, div.c-article-paywall",
          "",
          "main header + div"
        );
        ...
        func_post = function () {
          ...
          let ads = document.querySelectorAll(
            'div[style*=";min-height:"] > div[id^="iqadtile"]'
          );
        ...
        func_post = function () {
          document
            .querySelectorAll("div[data-carousel-id-slider]")
        ...
        if (paywall && dompurify_loaded) {
          ...
          if (spark_script) {
            ...
            if (match) {
              ...
              let url_src =
                "https://app-webview.sparknews.funkemedien.de/" +
                spark_domain +
        ...
        if (paywall) {
          ...
          let div_hidden = document.querySelector(
            "div.paywall-full-content[style]"
          );
          if (div_hidden) {
        ...
        matchDomain(de_madsack_domains) ||
        document.querySelector('head > link[href*=".rndtech.de/"]')
      ) {
        // plus code in contentScript_once_var.js (timing)
        ...
        matchDomain("ruhrnachrichten.de") ||
        document.querySelector('a.mgw-logo[href^="https://mgw.de"]')
      ) {
        ...
        getJsonUrl(
          "body.is_plus_article",
          { rm_class: "is_plus_article" },
          "article",
          { art_append: 1, art_hold: 1, art_class: "article__content" },
          article_id
        );
        ...
        matchDomain(de_vrm_domains) ||
        matchDomain(de_vrm_custom_domains)
      ) {
        let ads = "div.adSlot, div.loadingBanner";
        hideDOMStyle(ads);
      } else if (
        matchDomain(ch_media_domains) ||
        document.querySelector(
          'head > link[href*="/assets.static-chmedia.ch/"]'
        )
      ) {
        let infobox_body = document.querySelector("div.infobox__body");
        if (infobox_body) infobox_body.removeAttribute("class");
        let paywall = document.querySelector("div.dynamic-regwall");
        removeDOMElement(paywall);
      } else csDone = true;
    } else if (window.location.hostname.match(/\.(dk|fi|se)$/)) {
      ...
      if (matchDomain("berlingske.dk")) {
        ...
        if (paywall) {
          ...
          if (footer) removeDOMElement(footer.parentNode);
        }
        let ads = 'div[class$="-ad"]';
        hideDOMStyle(ads);
        let video_iframes = document.querySelectorAll(
          "div.embed-block > iframe[width][height]"
        );
        ...
        if (obscured) obscured.classList.remove("epfl-pw-obscured");
      } else csDone = true;
    } else if (
      window.location.hostname.match(/\.(es|pt|cat)$/) ||
      matchDomain([
        "diariocordoba.com",
        "diariovasco.com",
        "elconfidencial.com",
      ...
      if (matchDomain(["ara.cat", "arabalears.cat"])) {
        ...
        if (premium) premium.classList.remove("newsType__content--closed");
        ...
        if (window.location.pathname.endsWith(".amp.html")) {
          ...
          let ads =
            "div.edi-advertising, div.header-ad, aside.news-sponsored-content, div.report__wrapper";
          hideDOMStyle(ads);
        }
        ...
        if (window.location.pathname.endsWith(".amp.html")) {
          ...
          if (paywall) {
            removeDOMElement(paywall);
            let content_hidden = document.querySelector(
              "div.content-not-granted-paywall"
            );
            if (content_hidden)
        ...
        if (
          ...
          window.location.search.match(/(\?|&)outputType=amp/)
        ) {
          amp_unhide_access_hide(
            '="vip"], [amp-access="success"',
            '="NOT vip"], [amp-access="NOT success"'
          );
        } else {
          let paywall = document.querySelector(
            "div#ctn_freemium_article, div#ctn_premium_article"
          );
          removeDOMElement(paywall);
        }
      } else if (matchDomain(es_grupo_vocento_domains)) {
        let paywall_sel =
          "div.voc-paywall, div.container-wall-exclusive__content-login";
        let paywall = document.querySelector(paywall_sel);
        if (!window.location.pathname.endsWith("_amp.html")) {
          if (!matchDomain(["eldiariomontanes.es"])) {
            amp_redirect(
              paywall_sel,
              "",
              window.location.pathname.replace(".html", "_amp.html")
            );
          } else {
            if (paywall) {
              let url = window.location.href;
          ...
          let banners =
            "div.voc-advertising, div.voc-ob-wrapper, div.voc-discounts, div.ev-em-modal, span.mega-superior, div.v-adv";
          hideDOMStyle(banners);
        } else {
          amp_unhide_access_hide(
            "=\"result='ALLOW_ACCESS'\"",
          ...
          if (body_top) body_top.removeAttribute("id");
        }
      } else if (
        matchDomain(es_epiberica_domains) ||
        matchDomain(es_epiberica_custom_domains)
      ) {
        ...
        if (
          window.location.pathname.endsWith(".amp.html") ||
          ["amp.elperiodico.com", "amp.epe.es"].includes(
          ...
          document
            .querySelectorAll("div#the-most")
            .forEach((e) => e.removeAttribute("style"));
          let ads = "amp-next-page, span.ad-signature, div.wrap";
          hideDOMStyle(ads);
        } else {
          let ads =
            'div.commercial-up-full__wrapper, aside.ft-ad, div[class^="_mo_recs"]';
          hideDOMStyle(ads);
        }
        ...
        if (!window.location.hostname.startsWith("amp.")) {
          ...
          let paywall = document.querySelector(
            article_sel + " > div.g-premium-blocker"
          );
          if (paywall && dompurify_loaded) {
            removeDOMElement(paywall);
            let article = document.querySelector(article_sel);
                    ...
                    response.text().then((html) => {
                      if (html.match(/window\.__INITIAL_DATA__\s?=\s?/)) {
                        try {
                          ...
                          let json = JSON.parse(
                            html
                              .split(/window\.__INITIAL_DATA__\s?=\s?/)[1]
                              .split(";window.")[0]
                              .replace(/":undefined([,}])/g, '":"undefined"$1')
                          ).nodes;
                          let pars = [];
                            ...
                            if (par.html) {
                              ...
                              if (par.contents) {
                                ...
                                for (let elem of par.contents) {
                                  ...
                                  if (elem_new) par_new.appendChild(elem_new);
                                }
                              } else {
        ...
        if (paywall) {
          ...
          let json = document.querySelector(
            'script[type="application/ld+json"]:not([class])'
          );
          if (json) {
            let json_text = JSON.parse(json.text).description.replace(
              /&amp;nbsp;/g,
              ""
            );
        ...
        if (!window.location.pathname.includes("/amp/"))
          amp_redirect(
            ".bloqueio_exclusivos, .container_assinatura, .bloco_bloqueio",
            "",
            window.location.href.replace("/detalhe/", "/amp/")
          );
        else
          amp_unhide_access_hide(
            '="subscriber"',
            '="NOT subscriber"',
            "div.adbox, amp-consent, .detalheAds, .exclusivos_bar"
          );
      } else csDone = true;
    } else if (
      (window.location.hostname.endsWith(".fr") &&
        !matchDomain(["lemagit.fr"])) ||
      matchDomain(
        [
        ...
        ].concat(fr_groupe_nice_matin_domains)
      )
    ) {
      //france
      ...
      if (matchDomain("alternatives-economiques.fr")) {
        window.setTimeout(function () {
          ...
          if (data_ae_poool) data_ae_poool.removeAttribute("style");
        }, 500);
        ...
        if (window.location.pathname.endsWith(".html"))
          getJsonUrl("div.box-info.yellow", "", "div.editor", {
            art_append: 1,
          });
        ...
        let videos = document.querySelectorAll(
          'div > div#prisma-player-leader[data-ads-core*="Dailymotion"]'
        );
        for (let video of videos) {
          try {
            ...
            if (json && json.playerVideoId) {
              let iframe = document.createElement("iframe");
              iframe.src =
                "https://www.dailymotion.com/embed/video/" + json.playerVideoId;
              iframe.style =
                "height: " +
                video.offsetHeight +
                "px; width: " +
        ...
        if (window.location.pathname.endsWith(".amp")) {
          amp_unhide_access_hide(
            '="paywall.access OR cha.access"',
            '="NOT (paywall.access OR cha.access)"'
          );
        } else {
        ...
        window.setTimeout(function () {
          let paywalled_content = document.querySelector(
            "div.ch-paywalled-content"
          );
          if (paywalled_content) paywalled_content.removeAttribute("style");
        }, 500);
        ...
        if (window.location.hostname.startsWith("amp.")) {
          ...
          let hidden_images = document.querySelectorAll(
            'img[src^="data:image/"][data-src]'
          );
          for (let hidden_image of hidden_images)
            hidden_image.setAttribute(
              "src",
              hidden_image.getAttribute("data-src")
            );
        ...
        if (paywall) {
          removeDOMElement(paywall);
          let article = document.querySelector(
            "div.article-full__body-content"
          );
          if (article) {
        ...
        if (window.location.pathname.startsWith("/amp/")) {
          amp_iframes_replace();
          let qiota_script = document.querySelector(
            'amp-script.i-amphtml-layout-size-defined[src$="/qiota-amp.js"]'
          );
          if (qiota_script) {
            qiota_script.classList.remove("i-amphtml-layout-size-defined");
            let amp_images = document.querySelectorAll(
              "amp-img > img.i-amphtml-fill-content"
            );
            for (let elem of amp_images) elem.removeAttribute("class");
        ...
        let entry_reg_wall = document.querySelector(
          "div.entry_reg_wall[style]"
        );
        if (entry_reg_wall) entry_reg_wall.removeAttribute("style");
            ...
            if (json_script) {
              ...
              if (json) {
                ...
                if (json_text && content) {
                  ...
                  let hidden_images =
                    document.querySelectorAll("img[src][srcset]");
                  for (let elem of hidden_images)
                    elem.removeAttribute("srcset");
                  let entry_content = document.querySelector(
                    "div.entry-content[style]"
                  );
                  if (entry_content) entry_content.removeAttribute("style");
                }
              }
            }
          }
        ...
        if (paywall && dompurify_loaded) {
          ...
          if (article) {
            let resource_key =
              cs_param.resource_key ||
              "34e68a3419a876e36729503e2107dfa556e1a105892e27010130a30018ccbe60";
            let url = window.location.href.split([/\?#/])[0];
            let url_src =
              "https://api-graphql.lefigaro.fr/graphql?id=FigaroCoreMobile_resourceByUrl_persistent_" +
              resource_key +
              "&variables={%22url%22:%20%22" +
              url +
              "%22}";
            fetch(url_src)
              .then((response) => {
                if (response.ok) {
                  response.json().then((json) => {
                    try {
                      ...
                      if (pars.length) article.innerHTML = "";
                      let parser = new DOMParser();
                      for (let par of pars) {
                        let elem;
                        let sub_elem;
                        ...
                        if (
                          [
                            "Heading",
                            "Paragraph",
                            "ParagraphWithPaywall",
                          ].includes(par_type)
                        ) {
                          if (par.paywall) par = par.paywall;
                          if (par.text.replace(/&nbsp;/g, "")) {
                            ...
                            if (par_type === "Heading")
                              elem.style =
                                "font-weight: bold; font-size: 1.85rem;";
                          }
                        } else if (
                          ["Photo", "VideoFigaro"].includes(par_type)
                        ) {
                          if (par.thumbnail) par = par.thumbnail;
                          if (par.image) {
                            elem = document.createElement("p");
                            elem.className = "fig-paragraph";
                            let img = makeFigure(
                              par.image.url,
                              par.caption.replace(/<[^<]*>/g, "") +
                                " " +
                                par.credit,
                              "",
                          ...
                          link_elem.innerText = par.title.replace(
                            /<[^<]*>/g,
                            ""
                          );
                          ...
                          if (par.sourceCode) {
                            ...
                            let tweet_link = elem.querySelector(
                              'a[href^="https://twitter.com/"], a[href^="https://x.com/"]'
                            );
                            if (tweet_link) {
                          ...
                          if (par.id) {
                            elem = document.createElement("iframe");
                            elem.src =
                              "https://www.youtube.com/embed/" + par.id;
                          ...
                          qtext.style =
                            "font-weight: bold; font-size: 28px; margin: 15px 0px;";
                          let qcredit = document.createElement("p");
                          qcredit.innerText = par.credit
                            ? parseHtmlEntities(par.credit)
                            : "";
        ...
        let paywall = document.querySelector(
          "body.paywall, body.pw, body.softwall"
        );
        if (paywall) paywall.classList.remove("paywall", "pw", "softwall");
        let banners = document.querySelectorAll(
          "div#fix-pw, div.disposableBanner"
        );
        removeDOMElement(...banners);
      } else if (matchDomain(["lejdd.fr", "parismatch.com", "public.fr"])) {
        let banners = ".forbidden";
        let ads =
          'div[class^="lmn-"], div.premium-hidden, div.p-aside--placeholder, section.outbrain-container';
        hideDOMStyle(banners + ", " + ads);
        let bottom_hide = document.querySelector(
          '.cnt[data-poool-mode="hide"]'
        );
        if (bottom_hide) {
        ...
        if (paywall && dompurify_loaded) {
          ...
          if (article) {
            ...
            if (match) {
              let id = match[1];
              let url_base =
                "https://apps.lemonde.fr/aec/v1/premium-android-phone/article/";
                ...
                try {
                  if (data) {
                    ...
                    if (article_new) {
                      ...
                      article_new
                        .querySelectorAll("p")
                        .forEach((e) => (e.className = "article__paragraph"));
                      article_new
                        .querySelectorAll("h2")
                        .forEach((e) => (e.className = "article__sub-title"));
                      article_new
                        .querySelectorAll("h3.question")
                        .forEach((e) => (e.className = "article__question"));
                      article_new
                        .querySelectorAll("figure")
                        .forEach((e) => (e.style = "margin: 0px 10px;"));
                      article_new
                        .querySelectorAll(
                          "div.see-also-container, div.reference"
                        )
                        .forEach((e) => (e.style = "margin: 20px 0px;"));
                      let image_divs =
                        article_new.querySelectorAll("div.image");
                      ...
                      let inread = article_new.querySelectorAll(
                        "div.inread-container"
                      );
                      removeDOMElement(...inread);
                      let links = article_new.querySelectorAll(
                        'a[href^="lmfr://element/article/"]'
                      );
                      for (let elem of links) {
                        let url_link = elem.href.split(/[\?#]/)[0];
                        let url_match = url_link.match(
                          /lmfr:\/\/element\/article\/(\d+)/
                        );
                        if (url_match) {
                          let id = url_match[1];
                          let url_src = url_base + id;
        ...
        function lepoint_main() {
          function decryptVariable(a) {
            var t = ["point", "les", "payants", "top"],
              ...
              o = (function () {
                var o = [];
                for (var e = 0; e < 4; e++) o.push(n[e]), o.push(t[e]);
                return o;
              })(),
              e = {
                ...
                parse: function (o) {
                  ...
                  return (
                    e.iv && (t.iv = CryptoJS.enc.Hex.parse(e.iv)),
            ...
            return JSON.parse(
              CryptoJS.AES.decrypt(JSON.stringify(a), o.join(" "), {
                format: e,
          ...
          if (article && window.variable_article_poool)
            window.postMessage({
              type: "from_page",
              data: decryptVariable(window.variable_article_poool),
            });
        }
        if (!matchDomain(["journal.lepoint.fr"])) {
          ...
          let ads =
            'div[id*="WRAP_"], div#StickyPaywall, div#paywall-sticky, #article-body div.slotpub, div.sticky-block';
          hideDOMStyle(ads);
        } else {
          let url = window.location.href;
        ...
        if (paywall && dompurify_loaded) {
          ...
          if (notes) header_nofix(article, "", "BPC > no fix");
          else if (article_id && article) {
            let url_src =
              "https://dwh.lequipe.fr/api/v4/efr/news/" + article_id;
            fetch(url_src)
              .then((response) => {
                if (response.ok) {
                  response.json().then((json) => {
                    try {
                      let pars = json.items.filter(
                        (x) => x.layout === "article_body"
                      )[0].objet.paragraphs;
                      pars.shift();
                      article.innerHTML = "";
                      ...
                      addStyle(
                        'div.article__body > p.Paragraph {font-family: "DINNextLTPro-Regular", sans-serif; font-size: 18px; font-weight: 400; line-height: 26px;}'
                      );
                      let parser = new DOMParser();
                      for (let par of pars) {
                        let elem;
                        if (par.content) {
                          if (
                            (par.content.match(/(^<div|\/div>$)/g) || [])
                              .length !== 1
                          ) {
                            let elem_type = par.content.includes("div>")
                              ? "div"
                              : "p";
                            let doc = parser.parseFromString(
                              "<" +
                          ...
                          if (par.media.url && par.media.ratio) {
                            ...
                            if (!parseInt(ratio)) ratio = 1.5;
                            let url = par.media.url
                              .replace(/\\u002F/g, "/")
                              .replace("{width}", "400")
                              .replace("{height}", parseInt(400 / ratio))
                              .replace("{quality}", "75");
                            let caption =
                              par.media.legende && par.media.legende.length > 2
                                ? par.media.legende
                                : "";
                            elem = makeFigure(
                              url,
                            ...
                            par.media.__type === "video" &&
                            par.media.id
                          ) {
                            let url = par.media.image.url
                              .replace("{width}", "400")
                              .replace("{height}", 400)
                              .replace("{quality}", "75");
                            elem = makeFigure(url, par.media.legend);
                            let video_link = document.createElement("a");
                            video_link.href = video_link.innerText =
                          ...
                          !["article_paragraph_pub"].includes(par.__type)
                        )
                          console.log(par);
                        if (elem) article.appendChild(elem);
                      }
          ...
          window.setTimeout(function () {
            ...
            if (paywall && dompurify_loaded) {
              ...
              if (json_script) {
                try {
                  let json = JSON.parse(
                    json_script.text.split(filter)[1].split("};")[0] + "}"
                  );
                  let data_article = json.queries.filter(
                    (x) => x.state.data.stripes
                  )[0].state;
                  let url = window.location.href;
                  let url_loaded = data_article.data.path;
                  if (
                    url_loaded &&
                    (!url_loaded.slice(-7).match(/\d+/) ||
                      !url.includes(url_loaded.slice(-7)))
                  )
                    refreshCurrentTab();
                  else {
                    let json_text =
                      data_article.data.stripes[0].mainContent[0].data
                        .description;
                    ...
                    if (article) {
                      ...
                      let article_new = doc.querySelector("div");
                      let error_iframes = article_new.querySelectorAll(
                      ...
                      if (article.parentNode && article_new) {
                        article.parentNode.replaceChild(article_new, article);
                        let article_lastnode = document.querySelector(
                          ".post-paywall  > :last-child"
                        );
                        if (article_lastnode) {
                          article_lastnode.setAttribute(
                            "style",
                    ...
                    styleElem.innerText =
                      ".post-paywall::after {height: auto !important;}";
                  }
                } catch (err) {
                  console.log(err);
                }
        ...
        if (window.location.search.match(/(\?|&)amp/)) {
          let size_defined = document.querySelector(
            "amp-script.i-amphtml-layout-size-defined"
          );
          if (size_defined) size_defined.style = "overflow:visible !important;";
          let overlays = document.querySelectorAll(
            "section.learn_more, div.sidebar, div.menu-footer, div.tooltip_bib, footer.content-info"
          );
          removeDOMElement(...overlays);
        }
      } else if (matchDomain("letelegramme.fr")) {
        let paywall = document.querySelectorAll("div.tlg-paywalled");
        for (let elem of paywall) elem.classList.remove("tlg-paywalled");
        let ads = 'div[id^="pub_"]';
        hideDOMStyle(ads);
      } else if (matchDomain("lexpress.fr")) {
        let ads =
          'div[class^="block_pub"], div[class^="bottom-bar"], div.teads__block, div.ban-bottom, div[class^="placeholder--ban-atf"]';
        hideDOMStyle(ads);
        ...
        if (paywall && dompurify_loaded) {
          ...
          let article =
            cs_param.article_sel || document.querySelector(article_sel);
          if (article) {
            let url_src =
              "https://arc.api.liberation.fr/content/v4/?website=liberation&website_url=" +
              encodeURIComponent(window.location.pathname);
            let x_api_key =
              cs_param["x-api-key"] || "ejeePeingeitaegho3weengeeyohpu";
                        ...
                        if (["header", "raw_html", "text"].includes(par.type)) {
                          ...
                          if (par.raw_oembed && par.raw_oembed.html) {
                            if (!par.subtype === "twitter") {
                              ...
                              sub_elem.href = sub_elem.innerText =
                                par.raw_oembed.url;
                              sub_elem.target = "_blank";
                            }
                          }
                          ...
                          if (par.items) {
                            ...
                            sub_elem.appendChild(
                              document.createTextNode("Lire aussi")
                            );
        ...
        let ads =
          'div[class^="StickyAd"], div[class^="default__OutbrainWrapper"]';
        hideDOMStyle(ads);
        ...
        let banners = document.querySelectorAll(
          ".membership-promo-container, .login_form_litle"
        );
        ...
        if (paywall && dompurify_loaded) {
          ...
          if (article) {
            ...
            function show_data(article, body) {
              ...
              let article_new = doc.querySelector("div");
              let lazy_images = article_new.querySelectorAll(
            ...
            function fetch_data(limit, offset = 0) {
              let url_src =
                "https://mobile.marianne.net/premium?limit=" +
                limit +
                "&offset=" +
                offset;
              fetch(url_src)
                  ...
                  if (response.ok) {
                    response.json().then((json) => {
                      try {
                        ...
                        if (src_articles) {
                          let src_article = src_articles.filter(
                            (x) => x.urlWeb === url
                          )[0];
                          let ls_update = true;
                          if (src_article) show_data(article, src_article.body);
                          ...
                          if (ls_update) {
                            ...
                            localStorage.setItem(
                              "###_json",
                              JSON.stringify(ls_json_articles)
                            );
                          }
                        }
                      } catch (err) {
            ...
            let url = window.location.href.split(/[#\?]/)[0];
            let meta_date = document.querySelector(
              'head > meta[property="article:published_time"][content]'
            );
            let art_date = "";
            if (meta_date) art_date = meta_date.content.split("T")[0];
            ...
            if (ls_date) {
              ...
              if (ls_date <= art_date) fetch_data(limit_low);
              else {
                let art_data = ls_json_articles[url];
                if (art_data) show_data(article, art_data);
                else if (Object.keys(ls_json_articles).length < limit_high)
                  fetch_data(limit_high);
                else header_nofix(article, "", "BPC > no fix (source file)");
              }
            } else {
            ...
            if (json) {
              ...
              if (json_text && article) {
                ...
                if (par_last) {
                  ...
                  if (
                    json_text.replace(/<[^<]*>/g, "").includes(par_last_str)
                  ) {
                    par_last.innerText = json_text.substring(
                      json_text.indexOf(par_last_str)
                    );
                  } else {
                    article.innerHTML = " ";
                    let article_new = document.createElement("p");
        ...
        let paywall = document.querySelector(
          'div[data-pleco-poool^="paywall"]'
        );
        if (paywall) {
          let intro = document.querySelector(
            'div[data-pleco-transition="fade"]'
          );
          removeDOMElement(paywall, intro);
        ...
        if (window.location.hostname.startsWith("amp.")) {
          let pars = document.querySelectorAll(
            ".qiota_reserve > p, .qiota_reserve > h2"
          );
          let pars_text = [];
          for (let par of pars) {
            if (pars_text.includes(par.innerText)) removeDOMElement(par);
            else pars_text.push(par.innerText);
          }
          let sizer = document.querySelector(
            "div.article-content > amp-script > i-amphtml-sizer"
          );
          removeDOMElement(sizer);
          let replaced_content = document.querySelector(
            "div.i-amphtml-replaced-content"
          );
          if (replaced_content) replaced_content.removeAttribute("class");
        }
        ...
        if (paywall.length) {
          removeDOMElement(...paywall);
          let visible_premium = document.querySelectorAll(
            "div.visible-premium"
          );
        ...
        if (paywall && dompurify_loaded) {
          ...
          if (article) {
            let url_src =
              "https://apps.telerama.fr/tlr/v1/premium-android-tablet/element?id=" +
              encodeURIComponent(window.location.pathname);
              ...
              try {
                if (data) {
                  ...
                  if (article_new && article.parentNode) {
                    article_new
                      .querySelectorAll('a[href^="tlrm://element?id="]')
                      .forEach(
                        (e) =>
                          (e.href = decodeURIComponent(
                            e.href.split("tlrm://element?id=")[1]
                          ))
                      );
                    article_new
                      .querySelectorAll("figure > img[data-src]:not([src])")
        ...
        matchDomain("lamontagne.fr") ||
        document.querySelector(
          'head > meta[name="google-play-app"][content^="app-id=com.centrefrance"]'
        )
      ) {
      ...
      window.location.hostname.endsWith(".it") ||
      matchDomain([
        "eastwest.eu",
        "ilsole24ore.com",
        "italian.tech",
      ...
      if (matchDomain("corriere.it")) {
        if (window.location.pathname.endsWith("_amp.html")) {
          ...
          if (
            window.location.pathname.includes("_preview.shtml") &&
        ...
        if (paywall) {
          paywall.removeAttribute("style");
          paywall.classList.remove("paywall");
          let intro = document.querySelectorAll(
            "div#testo_articolo > p, div#testo_articolo > h3"
          );
        ...
        if (window.location.search.startsWith("?amp=1")) ampToHtml();
          ...
          if (paywall) {
            ...
            if (!window.location.search.startsWith("?reason=unauthenticated")) {
              window.location.href =
                window.location.pathname.replace("_preview", "") + "?gaa_at=g";
            } else {
              let json_script = getArticleJsonScript();
              ...
              if (json_script) {
                ...
                if (json) {
                  ...
                  let content = document.querySelector(
                    "div.content > p.has-first-letter"
                  );
                  if (json_text && content) {
                    let content_new = document.createElement("p");
                    content_new.innerText = json_text;
                    content.parentNode.replaceChild(content_new, content);
                    let article_body = document.querySelector(
                      "section.body-article"
                    );
                    if (article_body) article_body.style = "height: auto;";
                  } else header_nofix(header);
                }
              } else header_nofix(header);
            }
          }
        } else if (window.location.pathname.endsWith("_amp.shtml")) ampToHtml();
      } else if (matchDomain("ilfattoquotidiano.it")) {
        if (window.location.pathname.endsWith("/amp/")) {
          amp_unhide_subscr_section(
            "div#_4sVideoContainer, div#post-consent-ui"
          );
          let comments = document.querySelector("div.content.comments");
          removeDOMElement(comments);
          let logo = document.querySelector(
            'a > amp-img[src$="/logo-tablet.svg"]'
          );
          if (logo) {
            let logo_new = document.createElement("img");
            logo_new.src = logo
              .getAttribute("src")
              .replace("logo-tablet.svg", "logo-desktop.svg");
        ...
        if (window.location.pathname.endsWith("/amp/")) {
          amp_unhide_subscr_section(
            'amp-ad, [class^="adv-"], div#gmpVideoContainer'
          );
        } else {
          amp_redirect("div.paywall");
        ...
        let paywall = document.querySelector(
          'div[class*="before:bg-gradient-to-t"]'
        );
        if (paywall && dompurify_loaded) {
          removeDOMElement(paywall);
          ...
          if (article) {
            ...
            for (let script of scripts) {
              if (
                script.text.match(filter) &&
                script.text.includes("canonical_url")
              ) {
            ...
            let source_script = getSourceJsonScript(
              /^self\.__next_f\.push\(\[1,"\\u003c/
            );
            if (source_script) {
              let source_text = source_script.text
                .split(filter)[1]
                .split('"])')[0]
                .replace(/\\u003c/g, "<")
                .replace(/\\u003e/g, ">")
              ...
              let parser = new DOMParser();
              let doc = parser.parseFromString(
                "<div>" + DOMPurify.sanitize(source_text) + "</div>",
                "text/html"
              );
              let article_new = doc.querySelector("div");
              let figures = article_new.querySelectorAll("figure[style]");
              for (let elem of figures) elem.removeAttribute("style");
              article.innerHTML = "";
              article.appendChild(article_new);
            }
          }
        ...
        csDoneOnce = true;
        let ads =
          "div.background-adv, div.abox, div.ob-smartfeed-wrapper, div.s24_adb";
        hideDOMStyle(ads);
      } else if (
        matchDomain(["iltirreno.it", "lanuovasardegna.it"]) ||
        matchDomain([
          "gazzettadimodena.it",
          "gazzettadireggio.it",
          "lanuovaferrara.it",
        ...
        if (window.location.pathname.includes("/news/")) {
          ...
          window.setTimeout(function () {
            let banners = document.querySelectorAll(
              "div.MuiSnackbar-root, div.css-16cchgy"
            );
            removeDOMElement(...banners);
          }, 1000);
        }
        ...
        if (window.location.pathname.toLowerCase().includes("/amp/")) {
          ...
          if (noscroll) noscroll.removeAttribute("style");
        ...
        if (
          ...
          window.location.search.startsWith("?amp")
        ) {
          amp_unhide_access_hide(
            '="c.customGranted"',
            '="NOT c.customGranted"',
            "amp-fx-flying-carpet, .watermark-adv, .amp__watermark"
          );
        } else {
          amp_redirect(
            'div[data-testid="paywall-container"], div[class^="Paywall_paywall_"]',
            "",
        ...
        if (matchDomain(["huffingtonpost.it", "lastampa.it"])) {
          ...
          if (!amp)
            ...
            if (!mobile)
              addStyle(
                "img.i-amphtml-fill-content {min-height: 50% !important; min-width: 50% !important;}"
              );
          }
        } else {
          if (!amp) {
          ...
          if (json_script) {
            try {
              ...
              if (json) {
                ...
                if (json_text) paywall.innerText = json_text;
              }
            } catch (err) {
              console.log(err);
            }
        ...
        if (!window.location.pathname.startsWith("/amp/")) {
          let paywall = document.querySelector(
            'div[class^="MainTextTruncated_premium"]'
          );
          if (paywall && dompurify_loaded) {
            removeDOMElement(paywall);
            let article = document.querySelector(
              'div > div[class^="MainTextTruncated_truncatedContent"]'
            );
            if (article) {
              let json_script = document.querySelector("script#__NEXT_DATA__");
              if (json_script) {
      ...
      window.location.hostname.match(/\.(be|nl)$/) ||
      matchDomain([
        "artsenkrant.com",
        "lavenir.net",
      ...
      if (matchDomain(be_groupe_ipm_domains)) {
        ...
        if (paywall) {
          ...
          window.setTimeout(function () {
            ...
            if (div_hidden) div_hidden.classList.remove("is-hidden");
          }, 1000);
        }
        ...
        window.setTimeout(function () {
          ...
          func_post = function () {
            if (video) {
              let video_new =
                document.querySelector(
                  'div[id$="-streamone"], div[id^="video-player-"]'
                ) ||
                document.querySelector(
                  'article div[style^="background-color"]'
                );
              if (!video_new) {
                let empty_div = document.querySelector(
                  'div > div[aria-hidden="true"]:empty:not([class])'
                );
                if (empty_div) video_new = empty_div.parentNode;
              }
          ...
          let url = window.location.href;
          let article_sel =
            'article div[id], section div[id]:not([id^="warning"], [id^="ad_overlayer"]), main div[id]:not([id^="warning"], [id^="ad_overlayer"])';
          getArchive(
            url,
            'head > meta[name$="article_ispaidcontent"][content="true"]',
            "",
            article_sel
          );
        }, 1500);
        ...
        if (paywall && dompurify_loaded) {
          ...
          if (article) {
            ...
            if (content_script) {
              try {
                let content = decode_utf8(
                  atob(content_script.text.split(filter)[1].split('";')[0])
                );
                let parser = new DOMParser();
                let doc = parser.parseFromString(
                  "<div>" +
        ...
        getJsonUrl(
          "div.piano-article__paywall",
          "",
          "div.piano-article__content"
        );
        ...
        if (paywall && dompurify_loaded) {
          ...
          if (json_script) {
            try {
              ...
              let json = JSON.parse(json_script.text);
              json = json.filter(
                (x) => typeof x === "string" && x.startsWith("<p>")
              );
        ...
        if (matchDomain("beleggersbelangen.nl")) {
          ...
          if (paywall) {
            ...
            if (no_account) no_account.classList.remove("no-account");
            let content_inner = document.querySelector(
              "div.content-inner[style]"
            );
            if (content_inner) content_inner.removeAttribute("style");
          }
        } else {
          ...
          if (paywall) {
            ...
            if (html) html.removeAttribute("class");
            ...
            let intro = document.querySelectorAll(
              "div.article-body > p, div.article-body > style"
            );
          ...
          if (
            ...
            !matchDomain(["artsenkrant.com", "kw.be"]) &&
            window.location.href.match(/\/(\w+-){2,}/)
          ) {
            let lazy_images = document.querySelectorAll(
              'img[src^="data:image/"][data-lazy-src]'
            );
            for (let elem of lazy_images) {
        ...
        matchDomain(["lc.nl", "dvhn.nl"]) ||
        document.querySelector('head > link[href*=".ndcmediagroep.nl/"]')
      ) {
        ...
        window.setTimeout(function () {
          let paywall = document.querySelector(
            "div.premium-login-box_login-box"
          );
          if (paywall && dompurify_loaded) {
            removeDOMElement(paywall);
            let article = document.querySelector("article");
              ...
              if (json_script) {
                try {
                  ...
                  if (json) {
                    ...
                    if (
                      json &&
                      json.viewData.article &&
                      json.viewData.article.modules
                    ) {
                          ...
                          if (["body_text", "intro", "quote"].includes(type)) {
                            if (elem.text) {
                              ...
                              if (type === "intro") {
                                ...
                                if (intro) intro.style = "font-weight: bold; ";
                              } else if (type === "quote")
                            ...
                            let elem_images =
                              elem.images_portrait || elem.images_landscape;
                            if (elem_images && elem_images.length) {
                              for (let img of elem_images) {
                                let url = img.image.sizes.large;
                                let caption_text = img.credits
                                  ? img.credits.replace(/(\n|<[^<]*>)/g, "")
                                  : "";
                                item = makeFigure(url, caption_text, {
                                  style: "width: 100%;",
                                });
                                article.append(item);
                              }
                    ...
                    } else header_nofix("div.article-content_base");
                  }
                } catch (err) {
                  console.log(err);
                }
        ...
        if (sub) sub.click();
        func_post = function () {
          let shades = document.querySelectorAll(
            'div[style*="background-color"][style*=";width"]'
          );
          for (let elem of shades) elem.style.width = "85%";
          let lazy_images = document.querySelectorAll(
            'picture img[loading="lazy"][style]'
          );
          for (let elem of lazy_images) elem.style = "width: 95%;";
          let widgets = document.querySelectorAll(
            'div[old-src^="https://valley.ad.nl/widgets/"]:not([src])'
          );
        ...
        getArchive(
          url,
          article_sel + '[data-reduced="true"]',
          { rm_attrib: "data-reduced" },
          article_sel
        );
      } else if (matchDomain(nl_dpg_media_domains)) {
        let banners =
          'aside[data-temptation-position^="ARTICLE_"], div[data-temptation-position^="PAGE_"], div[class^="ad--"], div[id^="article_paragraph_"]';
        hideDOMStyle(banners);
      } else if (matchDomain("nrc.nl")) {
        let banners = document.querySelectorAll(
          'div[id$="modal__overlay"], div.header__subscribe-bar, div.banner'
        );
        removeDOMElement(...banners);
      } else if (matchDomain("telegraaf.nl")) {
        let premium = document.querySelector(
          'div[class^="Article__premium"] > label, div.PopupWrapper__paywall'
        );
        let paywall = document.querySelector(
          'data-hydrate[data-name="PaywallHandler"]'
        );
        let article =
          document.querySelector("section > div.DetailArticleImage") ||
          document.querySelector("section > p.Article__intro");
        ...
        if (premium && paywall && article && dompurify_loaded) {
          ...
          let par_style =
            "font-weight: normal; font-size: 16px; line-height: 1.5;";
          function show_text(window_text, div_main) {
            window_text = window_text
              .split("window.telegraaf.")[0]
              .replace(/(^\s?=\s?"|";$|\\")/gm, "")
              .replace(/\\\\u003c/gm, "<");
            let parser = new DOMParser();
            let doc = parser.parseFromString(
            ...
            for (let par of pars) par.style = "margin: 10px 0px;";
            let placeholders = article_new.querySelectorAll(
              "div.TeaserImage__placeholder"
            );
            for (let elem of placeholders) elem.removeAttribute("class");
            let media = article_new.querySelectorAll(
              "div.NewsletterForm, div.DetailArticleVideo"
            );
            removeDOMElement(...media);
            let twitter_quotes = article_new.querySelectorAll(
              "blockquote.twitter-tweet > a[href]"
            );
            ...
            if (mobile) {
              let art_images = article_new.querySelectorAll(
                "div.DetailArticleImage > div > img"
              );
              for (let elem of art_images) elem.style.width = "95%";
            }
            let error_iframes = article_new.querySelectorAll(
              'iframe[allow*="fullscreen"][allowfullscreen]'
            );
          ...
          if (
            window_script &&
            window_script.text.includes("window.telegraaf.articleBodyBlocks")
          ) {
            removeDOMElement(paywall);
            let window_text = window_script.text.split(
              "window.telegraaf.articleBodyBlocks"
            )[1];
            if (window_text) show_text(window_text, div_main);
          } else {
            removeDOMElement(paywall);
            let url = window.location.href.split(/[#\?]/)[0];
            fetch(url).then((response) => {
              if (response.ok) {
                response.text().then((html) => {
                  if (html.includes("window.telegraaf.articleBodyBlocks")) {
                    let window_text = html
                      .split("window.telegraaf.articleBodyBlocks")[1]
                      .split("</script>")[0];
                    if (window_text) show_text(window_text, div_main);
                  }
                });
        ...
        if (matchDomain("belegger.tijd.be")) {
          ...
          if (noscroll) noscroll.classList.remove("js-overflow-hidden");
          let inert = document.querySelectorAll("[inert]");
          for (let elem of inert) elem.removeAttribute("inert");
          let banners = document.querySelectorAll(
            'div[class^="paywall-banner__"]'
          );
          removeDOMElement(...banners);
        } else {
          ...
          if (paywall) {
            ...
            let popup = document.querySelector(
              'div[data-id="react-paywall-auth0"]'
            );
        ...
        window.setTimeout(function () {
          let paywall = document.querySelectorAll(
            'section[class^="c-paywall"]'
          );
          if (paywall.length && dompurify_loaded) {
            removeDOMElement(...paywall);
            let article = document.querySelector(
              "div.c-article-content__container"
            );
            if (article) {
              let json_script = document.querySelector("script#__NEXT_DATA__");
              if (json_script) {
        ...
        document.querySelector('head > link[href="//ppt.promedia.nl"]') ||
        document.querySelector(
          'head > script[src*="/pmgnews/scripts/promedia.js"]'
        )
      ) {
      ...
      window.location.hostname.match(/\.pl$/) ||
      matchDomain(["parkiet.com"])
    ) {
      //poland
      ...
      if (matchDomain("pb.pl")) {
        ...
        if (paywall) {
          paywall.classList.remove("paywall");
          let article_hidden = paywall.querySelector(
            "section.o-article-content"
          );
          if (article_hidden) article_hidden.removeAttribute("class");
          let loader = document.querySelector(
            "div.o-piano-template-loader-box"
          );
        ...
        let ads =
          'div.adPlaceholder , div[class^="Ad"][class*="Placeholder_"], div[data-placeholder-caption], div[data-run-module$=".floatingAd"], aside[data-ad-container], aside.adsContainer, [class^="pwAds"], .hide-for-paying, div.onet-ad, div.bottomBar, ad-default';
        hideDOMStyle(ads);
      } else if (matchDomain("polityka.pl")) {
        let paywall = document.querySelector("div.cg-article-salebox");
        if (paywall) {
          removeDOMElement(paywall);
          let elem_hidden = document.querySelectorAll(
            "div.cg_article_meat > [style]"
          );
          for (let elem of elem_hidden) elem.removeAttribute("style");
          let fade = document.querySelector("article.article_status-cut");
          if (fade) fade.classList.remove("article_status-cut");
        }
        ...
        let url = window.location.href;
        func_post = function () {
          let empty_spans = document.querySelectorAll(
            "figure > a > span:empty"
          );
        ...
        if (matchDomain(["wyborcza.biz", "wyborcza.pl"]))
          getArchive(
            url,
            "div.article--content-fadeout",
            { rm_attrib: "class" },
            'div.container[class*="pt"]',
            "",
            'div.body > div:not([style*="background-color:"]):not([old-position])'
          );
        else
          getArchive(
            url,
            "section.fade-out-article",
            { rm_attrib: "class" },
            "article"
          );
      ...
      (window.location.hostname.match(/\.(ie|uk)$/) &&
        !matchDomain(["vogue.co.uk"])) ||
      matchDomain([
        "apollo-magazine.com",
        "autosport.com",
        "decanter.com",
      ...
      if (matchDomain("apollo-magazine.com")) {
        ...
        let paywall = document.querySelector(
          "div.ms-block, div.register-block"
        );
        if (paywall) {
          removeDOMElement(paywall);
          let json_script = getArticleJsonScript();
          if (json_script) {
            ...
            if (json) {
              ...
              if (json_text && article) {
                ...
                article_new.innerText = json_text;
                let fade = document.querySelector(
                  'div.article-section > div[style*="max-height"]'
                );
                removeDOMElement(fade);
                let url = window.location.href;
                article.append(
        ...
        header_nofix(
          "div.ms-article-content > p",
          "div.ms-piano_article-banner"
        );
      } else if (matchDomain(["belfasttelegraph.co.uk", "independent.ie"])) {
        let paywall = document.querySelector('div[class*="_fadetowhite"]');
        if (paywall && dompurify_loaded) {
          let content = document.querySelector(
            'script[data-fragment-type="ArticleContent"]'
          );
          if (content) {
            removeDOMElement(paywall);
            ...
            let intro = document.querySelector(
              'div > div[data-auth-intro="article"]'
            );
            if (intro) {
              let intro_par = intro.querySelector("p[class]");
              let intro_par_class;
              if (intro_par) intro_par_class = intro_par.getAttribute("class");
                      ...
                      for (let type in par) {
                        ...
                        if (type === "bullet_list") {
                          ...
                          for (let sub_item of item) {
                            let li = document.createElement("li");
                            li.innerText = parseHtmlEntities(
                              sub_item.replace(/<[^<]*>/g, "")
                            );
                            ul.appendChild(li);
                          }
                          ...
                          !["ad", "quote", "streamone"].includes(type)
                        ) {
                          ...
                          let error_iframes = elem.querySelectorAll(
                            'iframe[allow*="fullscreen"][allowfullscreen]'
                          );
                          for (let iframe of error_iframes)
                            iframe.removeAttribute("allowfullscreen");
                        ...
                        window.setTimeout(function () {
                          if (elem) intro.parentNode.appendChild(elem);
                        }, 500);
                      }
                    }
                  }
        ...
        function bpie_main() {
          if ($) {
            ...
            let article_id;
            if (article_id_dom) article_id = article_id_dom.id;
            if (article_id) {
              let bp_ajaxurl =
                "https://www.businesspost.ie/wp-admin/admin-ajax.php";
              let data_ajax = {
        ...
        let paywall = document.querySelector(
          'div[id^="react_subscriber_content_"]'
        );
        if (paywall && dompurify_loaded) {
          removeDOMElement(paywall);
        ...
        func_post = function () {
          let lazy_images = document.querySelectorAll(
            'figure > picture > img[loading="lazy"][src^="data:image/gif"][new-cursrc]'
          );
          for (let elem of lazy_images) {
            elem.removeAttribute("loading");
          ...
          if (mobile) {
            let grids = document.querySelectorAll(
              'div[style*="grid-template-areas"], article#site-content'
            );
            for (let elem of grids) elem.style = "margin: 10px;";
          }
        };
        let url = window.location.href;
        getArchive(
        ...
        let banners =
          ".o-cookie-message, .js-article-ribbon, .o-ads, .o-banner";
        hideDOMStyle(banners);
        ...
        let banner = document.querySelector(
          'div[data-behaviour="react-paywall-threshold"]'
        );
        removeDOMElement(banner);
        ...
        if (entry_content)
          ...
          getJsonUrl(paywall_sel, "", "div.entry-content__wrapper", {
            art_append: 1,
            art_hold: 1,
            art_class: "entry-content",
          });
        let ads =
          '#subscribe-ribbon, div.ad-slot, div[style*="background-image: linear-gradient"]';
        hideDOMStyle(ads);
      } else if (matchDomain("stylist.co.uk")) {
        let paywall = document.querySelector(
          'div[data-testid="paywall-component"]'
        );
        if (paywall && dompurify_loaded) {
          removeDOMElement(paywall);
          let json_script = document.querySelector("script#__NEXT_DATA__");
                  ...
                  if (article) {
                    ...
                    for (let par of pars) {
                      ...
                      if (par.paragraph) {
                        ...
                        ["image", "interactive_image"].includes(
                          par.acf_fc_layout
                        )
                      ) {
                        let image_array = [];
                        if (par.image) image_array = [par.image];
                        ...
                        !["newsletter_signup", "pull-quote"].includes(
                          par.acf_fc_layout
                        )
                      )
                        console.log(par);
        ...
        getJsonUrl(
          "div.tls-single-article__closed-paywall",
          "",
          "div.tls-article-body",
        ...
        if (window.location.hostname !== "epaper.thetimes.com") {
          func_post = function () {
            ...
            let headers = document.querySelectorAll(
              'article:not([id]) div[style*="text-align"]'
            );
            ...
            for (let n = 0; n < 5; n++) {
              window.setTimeout(function () {
                ...
                for (let elem of page_scroll) elem.style.overflow = "auto";
              }, n * 500);
            }
          };
          if (!url.includes("?shareToken=")) {
            let teaser = document.querySelector(
              'body[data-view-name="teaser-article"]'
            );
            if (teaser) {
              getArchive(
                url,
                "div#paywall-portal-article-footer",
                "",
                "article:not([id])"
              );
            }
          }
          let paywall_page = document.querySelector(
            "div#paywall-portal-page-footer"
          );
          let block = document.querySelector(".subscription-block");
          removeDOMElement(paywall_page, block);
          let ads =
            '#ad-article-inline, div#sticky-ad-header, div[class*="InlineAdWrapper"], div[class*="NativeAd"], div.gyLkkj';
          hideDOMStyle(ads);
        }
        ...
        matchDomain(uk_nat_world_domains) ||
        document.querySelector(
          'footer > div a[href^="https://www.nationalworldplc.com"]'
        )
      ) {
        let ads =
          'div[class^="MarkupAds__Container-"], div[class*="_AdContainer-"], div[class^="Dailymotion__Wrapper-"], div.banner';
        hideDOMStyle(ads);
      } else csDone = true;
    } else if (
      window.location.hostname.match(/\.(ar|br|cl|mx|pe|uy)$/) ||
      matchDomain([
        "abcmais.com",
        "cambiocolombia.com",
        "clarin.com",
      ...
      if (matchDomain("abcmais.com")) {
        if (!window.location.pathname.endsWith("/amp/")) {
          getJsonUrl(
            "section#section-iframe-assinante",
            "",
            "div.degressing-opacity"
          );
        } else {
        ...
        let ads =
          'div.ad-slot, div.box-adv, div.wrapperblock, div.banner, div[id^="div-gpt-ad-flotante"]';
        hideDOMStyle(ads);
        let ads_inline = document.querySelectorAll(
          'div > div.sticky, div > div.SRA, div > div[id^="div-gpt-ad-inread"], div > div[id^="div-gpt-ad-caja"], div > div[id^="div-gpt-ad-horizontal"]'
        );
        for (let ad of ads_inline) hideDOMElement(ad.parentNode);
      } else if (matchDomain("cambiocolombia.com")) {
        if (!window.location.pathname.startsWith("/amp/")) {
          amp_redirect(
            "div#require-access",
            "",
            "/amp" + window.location.pathname
          );
        } else {
          amp_unhide_subscr_section();
        }
          ...
          if (paywall) {
            ...
            if (json_script) {
              try {
                let json = JSON.parse(json_script.text);
                if (json) {
                  let json_text = json[1].articleBody.replace(
                    /\s{2,}/g,
                    "\r\n\r\n"
                  );
                  let content = document.querySelector(
                    "section.s-content__text"
                  );
                  if (json_text && content) {
                    content.innerHTML = "";
                    let article_new = document.createElement("p");
            ...
            if (content_soft) {
              ...
              let freemium = document.querySelectorAll(
                'div[class^="s-freemium"], div.maggazine-add'
              );
        ...
        let ads =
          "div#ad-slot-header, div.ad-slot-intext, div#selectMediaNota, div.b-suscription-container";
        hideDOMStyle(ads);
      } else if (matchDomain("crusoe.com.br")) {
        getJsonUrl("section.paywall", "", "div#content_post", {
          art_append: 1,
        });
        let ads =
          'div#gpt-leaderboard, div.ads_desktop, div[class^="container-banner-"], div.catchment-box';
        hideDOMStyle(ads);
        ...
        if (paywall) {
          ...
          if (fade) fade.classList.remove("story-contents--fade");
        }
        ...
        if (window.location.search.includes("outputType=amp")) {
          amp_unhide_subscr_section(
            '[class^="Widget"], amp-fx-flying-carpet',
            false
          );
          let googledoc_iframes = document.querySelectorAll(
            'div > amp-iframe[src^="https://docs.google.com/viewer"][class]'
          );
        ...
        if (window.location.pathname.endsWith("/amp")) {
          ...
          let amp_images = document.querySelectorAll(
            "div.fixed-container > amp-img.null"
          );
          for (let amp_image of amp_images) {
            let elem = document.createElement("img");
            Object.assign(elem, {
        ...
        if (modulos) modulos.classList.remove("modulos");
        let ads = '[class^="c-add"]';
        hideDOMStyle(ads);
        ...
        if (!window.location.pathname.endsWith("/amp.html")) {
          ...
          if (compress_text) compress_text.classList.remove("compress-text");
        }
      } else if (matchDomain("estadao.com.br")) {
        if (
          window.location.pathname.match(/(\.amp$|^\/amp\/)/) ||
          window.location.search.startsWith("?amp")
        ) {
          amp_unhide_access_hide(
            "=\"outputValue='hide_paywall'\"",
            "=\"outputValue='show_paywall'\"",
            'amp-fx-flying-carpet, div[class^="pAd"], div.ads-container'
          );
        } else {
          let paywall = document.getElementById(
            "paywall-wrapper-iframe-estadao"
          );
          removeDOMElement(paywall);
        ...
        if (matchDomain("folha.uol.com.br")) {
          ...
          let hidden_images = document.querySelectorAll(
            'div[id^="attachment_"] > a > img[src^="http:"][srcset]'
          );
          for (let hidden_image of hidden_images) {
        ...
        let ads =
          'div[class*="advertising"], div.jupiter-ads, div.up-floating, div[data-cp-id$="asfads"]';
        hideDOMStyle(ads);
      } else if (matchDomain("gauchazh.clicrbs.com.br")) {
        window.setTimeout(function () {
          let blink = document.querySelector(
            "div:not(.hidden) > div:not([class]) > div.animate-pulse"
          );
          if (blink) {
            csDoneOnce = true;
            refreshCurrentTab();
        ...
        } else if (window.location.pathname.includes("/amp/")) ampToHtml();
        if (!window.location.pathname.includes("/amp/")) {
          let ads =
            'div[id^="ad-container"], div.content-ads, div[class^="block__advertising"], div#pub-in-text-wrapper, div.area_publicidade_container';
          hideDOMStyle(ads);
        }
        ...
        if (window.location.search.startsWith("?display=amp")) {
          ...
          let banners = document.querySelectorAll(
            "div.softwall, div.subscribe-notice"
          );
          removeDOMElement(...banners);
        }
        ...
        if (window.location.pathname.startsWith("/revista/")) {
          ...
          if (loading_content) loading_content.removeAttribute("class");
          ...
          if (div_expandable) div_expandable.classList.remove("expandable");
          let ads = "section.ad-wrapper, div.autozep-outer";
          hideDOMStyle(ads);
        }
      } else if (
        window.location.hostname.endsWith(".cl") &&
        document.querySelector(
          'head > meta[property="og:image"][content*="://impresa.soy-chile.cl/"]'
        )
      ) {
        window.setTimeout(function () {
          let content = document.querySelector("div.content");
          if (content) content.setAttribute("id", "content_new");
        ...
        waitDOMAttribute(
          ...
          "BODY",
          "class",
          (node) => node.removeAttribute("class"),
          true
        );
        csDoneOnce = true;
      } else csDone = true;
    } else {
      //other (like com/org & not ar/at/au/be/br/cat/ch/cl/de/dk/fi/fr/es/ie/nl/pe/pt/se/uk/uy))
      ...
      if (matchDomain(usa_adv_local_domains)) {
        if (!window.location.search.startsWith("?outputType=amp")) {
          ...
          if (paywall && article && dompurify_loaded) {
            ...
            let fusion_script = document.querySelector(
              "script#fusion-metadata"
            );
            if (fusion_script) {
              paywall.classList.remove("paywall");
              ...
              amp_redirect(
                paywall_sel,
                "",
                window.location.pathname + "?outputType=amp"
              );
          }
        }
        ...
        matchDomain(usa_arizent_custom_domains)
      ) {
        ...
        getJsonUrl("div.subscribeWrapper", "", "div.post_teaser", {
          art_append: 1,
          art_hold: 1,
        });
      } else if (
        matchDomain(["barandbench.com", "theleaflet.in", "thenewsminute.com"])
      ) {
        ...
        if (window.location.pathname.includes("/subscriber-only/")) {
          ...
          window.setTimeout(function () {
            let dialog = document.querySelector(
              'div[id^="headlessui-dialog-"], div.sheet-overlay'
            );
            if (dialog) {
              ...
              let overlays = document.querySelectorAll(
                "html.is-overlaid, body.is-overlaid"
              );
        ...
        let ads =
          'div[data-ad-status], div[data-ad-type], div[class*="FullWidthAd_"], div.adWrapper';
        ...
        waitDOMAttribute(
          "body",
          "BODY",
          "data-paywall-overlay-status",
          (node) => node.removeAttribute("data-paywall-overlay-status"),
          true
        );
          ...
          if (json_script) {
            try {
              ...
              if (json && json.props.pageProps.data.htmlContent) {
                ...
                let content = document.querySelector(
                  'div[class^="MainStory_storycontent__"'
                );
                if (json_text && content) {
                  content.innerHTML = "";
        ...
        let paywall = document.querySelector(
          "div.piano-paywall-container[hidden]"
        );
        ...
        if (paywall) {
          ...
          if (json_script) {
            ...
            if (json) {
              let json_text = json.articleBody;
              let img_main = document.querySelector(
                "div.story--header picture > img[src]"
              );
        ...
        removeDOMElement(preview);
        let article_hidden = document.querySelector(
          'div[class~="contentBody" i][hidden]'
        );
        if (article_hidden) {
          let attributes = [...article_hidden.attributes].filter(
            (x) => x.name !== "class"
          );
        ...
        let subwall = document.querySelector(
          'div[data-component-id="subwall"]'
        );
        if (subwall) {
        ...
        let regwall_keys = Object.keys(window.localStorage).filter((x) =>
          x.match(/reg_?wall/i)
        );
        ...
        let paywall = document.querySelector(
          'div[class^="ContentGate_wrapper__"]'
        );
        if (paywall && dompurify_loaded) {
          removeDOMElement(paywall);
        ...
        if (window.location.pathname.match(/^(\/amp)?\/article\//)) {
          if (!window.location.search.startsWith("?rel=plus")) {
            ...
            let photo_links = document.querySelectorAll(
              'div > a[href^="' +
                window.location.pathname.replace(/\/\d{8,}/, "") +
                '"]'
            );
            for (let elem of photo_links) elem.href += "?rel=plus";
            let close_button = document.querySelector(
              "header > div > span > svg"
            );
            if (!document.querySelector("a#bpc_close") && close_button) {
              let elem = document.createElement("a");
              elem.href = window.location.pathname
                .split("?")[0]
                .replace(/\/\d{8,}/, "");
              elem.id = "bpc_close";
        ...
        if (paywall) {
          ...
          if (content && full_text) content.innerText = full_text.innerText;
          let page_content = document.querySelector(
            "div.pageContent:not([style])"
          );
          if (page_content)
        ...
        if (paywall) {
          let section_words = pageContains(
            "p",
              ...
              try {
                ...
                if (json) {
                  ...
                  if (body) {
                    ...
                    let parser = new DOMParser();
                    let doc = parser.parseFromString(
                      '<div class="w" style="font-size: 18px; line-height: 30px; position: relative;">' +
                        DOMPurify.sanitize(body, dompurify_options) +
                        "</div>",
                      "text/html"
                    );
                    ...
                    for (let par of pars) par.style = "margin: 20px 0px;";
                    article.appendChild(article_new);
                  }
                }
              } catch (err) {
        ...
        if (paywall && dompurify_loaded) {
          removeDOMElement(paywall);
          let filter = /\\"content\\":\[/;
          let json_script = getSourceJsonScript(
            filter,
            ":not([id], [src], [type])"
          );
          if (json_script) {
                  ...
                  for (let pars of json)
                    for (let par of pars) {
                      if (!par.match(/^\$\w{2}$/)) {
                        ...
                        let img_srcset_drop = elem.querySelectorAll(
                          "figure > img[srcset]"
                        );
        ...
        let ads_selector = window.location.pathname.endsWith("/lite/")
          ? ".ad-bg-container"
        ...
        window.setTimeout(function () {
          ...
          let msg_bottom = document.querySelector(
        ...
        if (window.location.pathname.match(/\/amp(\/)?/)) {
          amp_unhide_access_hide(
            '="NOT p.showRegWall AND NOT p.showPayWall"',
            '="p.showPayWall"',
            '[class^="amp-ad"], div.paywall'
          );
        } else {
        ...
        if (window.location.pathname.match(/\/ty-article/)) {
          ...
          if (!mh) {
            ...
            if (article) {
              ...
              mh_new.style =
                "font-size: 20px; font-weight: bold; text-align: center; margin: 20px;";
        ...
        let paywall = document.querySelector(paywall_sel);
        if (dompurify_loaded) {
          if (paywall) hilltimes_main(paywall);
          else waitDOMElement(paywall_sel, "DIV", hilltimes_main, false);
          csDoneOnce = true;
        }
        ...
        if (paywall) {
          ...
          if (freemium_text) freemium_text.classList.remove("freemiumText");
        }
        let noscroll = document.querySelector("body.open-popup");
        if (noscroll) noscroll.classList.remove("open-popup");
        let close_story = ".closeStory";
        let ads = 'div[class^="adHeight"]';
        hideDOMStyle(close_story + ", " + ads);
      } else if (matchDomain("historyextra.com")) {
        let article_masked = document.querySelector(
          ".template-article__masked"
        );
        if (article_masked) {
          let extra_pars = document.querySelectorAll(
            "div.template-article__masked > p"
          );
        ...
        if (url.includes(".com/wap/#/content/")) {
          ...
          let paywall = document.querySelector(
            "div.nfzm-article-jumbotron--unauth"
          );
          if (paywall) {
            removeDOMElement(paywall);
        ...
        if (paywall) {
          paywall.removeAttribute("class");
          let gradient_container = document.querySelector(
            "div.gradient-container"
          );
          removeDOMElement(gradient_container);
        }
        let what_is_inkl = document.querySelector(
          ".what-is-inkl-container, .features-panel"
        );
        let signup = document.querySelectorAll(
          '.article-signup-container, .locked-sign-up-container, div[class*="/inkl-watermark.svg"]'
        );
        let shared_banner = document.querySelector(
          "div.shared-article-inline-banner"
        );
        removeDOMElement(what_is_inkl, ...signup, shared_banner);
        let dismiss_button = document.querySelector(
          "div.dismiss-button-container button.btn"
        );
        ...
        if (paywall) {
          ...
          for (let elem of blurred) elem.classList.remove("blurFilter");
        }
          ...
          if (json_script) {
            try {
              ...
              if (
                ...
                json.props.pageProps.post.content
              ) {
                ...
                if (article) {
                  ...
                  let parser = new DOMParser();
                  let doc = parser.parseFromString(
                    "<div>" + DOMPurify.sanitize(article_new) + "</div>",
                    "text/html"
                  );
                  let content_new = doc.querySelector("div");
                  article.appendChild(content_new);
        ...
        window.setTimeout(function () {
          ...
          if (paywall) {
            ...
            if (article)
              article.firstChild.before(
                document.createTextNode("Refresh page or"),
                archiveLink(url)
              );
          }
        }, 5000);
        ...
        let premium_banners = document.querySelectorAll(
          ".hide-for-premium, #hiddenPremiumForm, #hiddenLink"
        );
        ...
        if (paywall) {
          let intro = document.querySelector(
            amp ? "div.story" : "div.details-story-wrapper"
          );
          removeDOMElement(paywall, intro);
          let paywall_content = document.querySelector(
            "div.paywall-content.hide"
          );
          if (paywall_content)
            paywall_content.className = amp
              ? ""
              : "news_details_page_row2 details-story-wrapper";
        }
        ...
        if (!window.location.pathname.startsWith("/amp/")) {
          ...
          let container_sponsored = document.querySelector(
            "div.container--sponsored"
          );
        ...
        window.setTimeout(function () {
          let url = window.location.href;
          let subscribed = document.querySelector(
            "div.Article__Content--gated"
          );
          ...
          let overlay = document.querySelector(
            "div.Article__Content__Overlay--gated"
          );
          if (overlay)
        ...
        if (!window.location.pathname.endsWith("/amp/")) {
          ...
          let paywall = document.querySelector(paywall_sel);
          if (paywall) {
            let amphtml = document.querySelector(
        ...
        let banners = document.querySelectorAll(
          'div[class^="a__sc-np"], div.subscibe-bar'
        );
        removeDOMElement(...banners);
        ...
        if (window.location.pathname.startsWith("/amp/")) {
          ...
          window.setTimeout(function () {
            amp_redirect(
              'div[class^="hard-paywall"], div[class*="geotag-container_"]'
            );
          }, 1000);
        }
              ...
              try {
                ...
                if (json && json.datePublished) {
                  let date = json.datePublished
                    .split(/T\d/)[0]
                    .replace(/-/g, "/");
                  let path_new = window.location.pathname.split(
                    /\/article\/(\d+-|mg\d+-\d+-)/
                  )[2];
                  if (path_new) {
                    let url =
                      "https://appan.newscientist.com/" +
                      date +
                      "/" +
                      path_new +
                      "content.html";
        ...
        if (window.location.pathname.startsWith("/amp/")) {
          amp_unhide_access_hide(
            "=\"AccessLevel = 'Full Content Access' OR Error = true\"",
            "=\"Error != true AND UserState != 'Subscribed'\""
          );
        } else {
        ...
        if (!window.location.pathname.startsWith("/athletic/")) {
          ...
          csDoneOnce = true;
          let banners =
            'div[data-testid="inline-message"], div[id^="ad-"], div.pz-ad-box';
          hideDOMStyle(banners);
        }
        ...
        if (paywall && dompurify_loaded) {
          ...
          if (json_script) {
            ...
            if (
              json &&
              json.props.initialState.dashboard.ARTICLE_POST_DETAIL_API.data
      ...
      } else if (matchDomain("ground.news")) {
        window.localStorage.removeItem("articlesAccessed");
        hideDOMStyle(
          'div.sticky.z-1000, div.fixed.z-\\[1000\\][style*="bottom"]',
          1
        );
        hideDOMStyle('div[class*="hideElementSiteWide"]', 2);
        ...
        if (mask_image) mask_image.classList.remove("jhqnqh");
        let read_more_buttons = document.querySelectorAll(
          "button.puppeteer_test_read_more_button.qu-bg--gray_ultralight"
        );
        for (let elem of read_more_buttons) elem.click();
        let overlay_cards = document.querySelectorAll(
          'div[class*="OverlayCard"]'
        );
        for (let elem of overlay_cards) elem.removeAttribute("class");
        window.setTimeout(function () {
          let answers = document.querySelectorAll(
            'div[class*="dom_annotate_question_answer_item_"]'
          );
          for (let answer of answers) {
            let wall = answer.querySelector("div.content-monetization-wall");
            if (wall) {
              wall.classList.remove("content-monetization-wall");
              let timestamp_link = answer.querySelector(
                "a.answer_timestamp[href]"
              );
              if (timestamp_link) {
        ...
        let ads =
          'div[data-testid="ResponsiveAdSlot"], div[data-testid="Dianomi"]';
        hideDOMStyle(ads);
        ...
        getArchive(
          url,
          'div[class^="Paywall_paywall_"]',
          "",
          'div[class^="Article_body_"]',
          "",
          'main div[style*="grid-row-start:body"]'
        );
        ...
        if (window.location.pathname.match(/^\/plus\/\w/)) {
          ...
          let lazy_images = document.querySelectorAll(
            "figure > img.lazy[data-src]:not([src])"
          );
          for (let elem of lazy_images) {
            elem.src = elem.getAttribute("data-src");
            elem.removeAttribute("class");
          }
        } else csDoneOnce = true;
            ...
            if (article) {
              ...
              if (json_script) {
                try {
                  ...
                  if (
                    json &&
                    json.props.pageProps.payload.data.article.body.json
                  ) {
                    let pars =
                      json.props.pageProps.payload.data.article.body.json;
                      ...
                      if (par.type === "p") {
                        for (let sub_elem of par.children) {
                          if (sub_elem.type === "text") {
                            if (sub_elem.data)
                              elem.appendChild(
                                document.createTextNode(sub_elem.data)
                              );
                          } else if (
                            ["a", "em", "span", "strong"].includes(
                              sub_elem.type
                            )
                          ) {
                            let first_child = sub_elem.children
                              ? sub_elem.children[0]
                              : "";
                            if (first_child) {
                        ...
                        if (par.attribs) {
                          if (par.attribs.class === "image-inline-container") {
                            ...
                            par.attribs.class.match(
                              /(methode-html|oembed|video)-wrapper/
                            )
                          ) {
                            for (let sub_elem of par.children) {
                              if (sub_elem.type === "iframe") {
                                ...
                                if (attribs.src) {
                                  ...
                                  if (attribs.width && attribs.height) {
                                    let ratio =
                                      attribs.width / article.offsetWidth;
                          ...
                          } else if (par.attribs.class) console.log(par);
                        }
                      }
                      if (elem.hasChildNodes()) article.appendChild(elem);
                    }
        ...
        getArchive(
          url,
          'div[id][data-sdkids-campaignname^="OVR_Anon_Locked_"]',
          "",
          "main"
        );
        ...
        if (window.location.pathname.startsWith("/article/")) {
          window.setTimeout(function () {
            ...
            if (pars.length && pars.length < 7) refreshCurrentTab_bg();
          }, 1000);
        }
        ...
        if (paywall) {
          ...
          removeDOMElement(intro);
          waitDOMElement("div#paywall_wrap", "DIV", (node) =>
        ...
        if (html_noscroll) html_noscroll.removeAttribute("class");
        ...
        let paywall = document.querySelector(
          "div.members-only-overlay-wrapper"
        );
        if (paywall && dompurify_loaded) {
          removeDOMElement(paywall);
          ...
          if (json_url_dom) {
            ...
            fetch(json_url).then((response) => {
              if (response.ok) {
                response.json().then((json) => {
                  try {
                    let json_text = json.acf.article_modules[0].copy.replace(
                      /data-src/g,
                      "src"
                    );
                    let content = document.querySelector(
                      "div.article__content div.text-block__inner"
                    );
                    if (json_text && content) {
                      let parser = new DOMParser();
                      let doc = parser.parseFromString(
        ...
        matchDomain(["techtarget.com", "computerweekly.com", "lemagit.fr"])
      ) {
        let paywall = document.querySelector("div.paywall");
        if (paywall) {
          paywall.classList.remove("paywall");
          let banners = document.querySelectorAll(
            "p#firstP, div#inlineRegistrationWrapper"
          );
          removeDOMElement(...banners);
        }
      } else if (matchDomain("tempo.co")) {
        let article_hidden = document.querySelector(
          "article > div:not([class]) div.overflow-hidden"
        );
        if (article_hidden) article_hidden.removeAttribute("class");
        ...
        if (paywall) {
          ...
          let img_dark = document.querySelector(
            "div.c-hero-article__image-img.o-image"
          );
        ...
        getJsonUrl("div.article--cropped", "", "div#body-copy", {
          art_append: 1,
        });
        ...
        if (paywall) {
          removeDOMElement(paywall);
          let json_script = document.querySelector(
            'script[displayName="initialState"]'
          );
          if (json_script) {
            let json_str = json_script.text.substring(
              json_script.textContent.indexOf("{")
            );
            try {
              ...
              if (json.body) {
                ...
                if (pars) {
                  ...
                  if (mobile_doc) {
                    let mobile_doc_text = mobile_doc.innerText.replace(
                      /(\r|\n)/g,
                      ""
                    );
                      ...
                      if (elem[0] === 1) {
                        ...
                        if (cards && cards[elem[1]]) {
                          ...
                          if (card[0] === "pt-image") {
                            ...
                            par_link.innerText =
                              card[1].linksData[0].long_headline;
                            par_elem.appendChild(par_link);
                          }
                        }
                      }
        ...
        if (matchDomain("magazine.thediplomat.com")) {
          ...
          if (article) article.classList.remove("h-96");
        ...
        if (!window.location.pathname.endsWith("/amp/")) {
          let counter = "#test";
          let ads =
            'div.ad, div.article-ad, div.dfp-ad, div#paywallbox, div[id^="piano-art-"]';
        ...
        if (paywall) {
          ...
          for (let elem of blureffect) elem.classList.remove("blureffect");
        }
              ...
              if (json && json.props.pageProps.post.fields) {
                ...
                window.setTimeout(function () {
                  ...
                  if (article) {
                    ...
                    let fade = document.querySelectorAll(
                        ...
                        if (par.content && par.content.length) {
                          for (let item of par.content) {
                            if (item.nodeType === "paragraph") {
                              elem.style =
                                "margin: 0px 20px; font-style: italic;";
            ...
            if (json) {
              ...
              if (json_text && article) {
                ...
                for (let elem of split) {
                  if (
                    !elem.includes("(function(") &&
                    !article_text.includes(elem)
                  ) {
                    let par_new = document.createElement("p");
        ...
        let hidden_images = document.querySelectorAll(
          'img.b-lazy[src^="data:image/"][data-src]'
        );
        for (let hidden_image of hidden_images) {
          hidden_image.setAttribute(
            "src",
        ...
        if (matchDomain("epaper.indiatimes.com")) {
          ...
          if (
            window.location.pathname.startsWith(
              "/english-news-paper-today-toi-print-edition/"
            )
          ) {
              ...
              if (json_script) {
                ...
                if (json) {
                  ...
                  if (json_text && content) {
                    ...
                    content.innerHTML = "";
                    addStyle(
                      '[type="synopsis"]::after {background: none !important;}'
                    );
                    content.appendChild(article_new);
                  }
                }
          ...
          if (!window.location.pathname.includes("/amp_")) {
            ...
            if (paywall && dompurify_loaded) {
                ...
                try {
                  ...
                  if (json) {
                    ...
                    if (json_text && article) {
                      ...
                      addStyle(
                        "div.paywall::after {background-image: none !important;}"
                      );
                    }
                  }
                } catch (err) {
              ...
              window.setTimeout(function () {
                let popup_button = document.querySelector(
                  "div.primeshow button:not([data-type])"
                );
                if (popup_button) popup_button.click();
              }, 1000);
        ...
        if (matchDomain("tradewindsnews.com")) {
          ...
          window.setTimeout(function () {
            let paywall = document.querySelector(
              "div.dn-paywall > div#sub-paywall-container"
            );
            if (paywall && dompurify_loaded) {
              removeDOMElement(paywall.parentNode);
              ...
              if (json_script) {
                try {
                  ...
                  if (article_id_index) {
                    ...
                    if (
                      article_id &&
                      !window.location.pathname.endsWith(article_id)
                    ) {
                      refreshCurrentTab();
                      return;
                    }
                  ...
                  for (let par of pars) {
                    ...
                    if (par && par.type) {
                      ...
                      if (["text", "subhead"].includes(type)) {
                        ...
                        elem = document.createElement("p");
                        if (par.title) elem.innerText = pars[par.title];
                        if (par.html) {
                          let content_new = parser.parseFromString(
                            "<div>" +
        ...
        let banners =
          'aside.paywall-bar, div[class^="MessageBannerWrapper-"], div.ad-stickyhero, div.ad_wrapper';
        hideDOMStyle(banners);
        ...
        if (matchDomain("european-rubber-journal.com")) {
          ...
          let sponsored_article = document.querySelector(
            "div.sponsored-article"
          );
        ...
        if (window.location.hostname.startsWith("amp.")) {
          ...
          if (roadblock) {
            ...
            article_next = document.querySelector(
              "article.next-in-depth-story > div.article-inner"
            );
            if (article_next) {
                  ...
                  function makeElem(elem, par_elem) {
                    if (Array.isArray(elem) && elem.length) {
                      ...
                      if (typeof item === "string") {
                            ...
                            if (img_data && img_data.type === "image") {
                              if (
                                ...
                                if (img_data.props.dangerousCaption) {
                                  caption_text =
                                    img_data.props.dangerousCaption.replace(
                                      /<\/?\w+>/g,
                                      ""
                                    );
                        ...
                        if (!item.class) console.log(item);
                        elem.shift();
                        makeElem(elem, par_elem);
                      }
                    } else if (
        ...
        if (
          window.location.pathname.startsWith("/comments") &&
          window.location.search.startsWith("?storyUrl=")
        ) {
          let wrapper = document.querySelector("div#comments-wrapper");
          if (wrapper) {
            let com_link = document.createElement("a");
            com_link.href = window.location.href.replace(
              "https://www.washingtonpost.com/comments?storyUrl",
          ...
          let body = document.querySelector("body");
          if (body) body.style = "margin: 10px;";
          let nav = document.querySelector("main > nav");
          if (nav && window.location.search.startsWith("?storyURL=")) {
            let com_link = document.createElement("a");
            com_link.href = decodeURIComponent(
              window.location.href.split("?storyURL=")[1]
            );
        ...
        if (!matchDomain("cn.wsj.com")) blockJsReferrer();
              ...
              if (paywall && dompurify_loaded) {
                if (!matchDomain("cn.wsj.com")) {
                  ...
                  func_post = function () {
                    ...
                    let inline_videos =
                      document.querySelectorAll('div[id^="video"]');
                        ...
                        try {
                          ...
                          if (video_data && inline_video.parentNode) {
                            ...
                            elem.style =
                              ...
                              "px; height: " +
                              (inline_video.offsetWidth * 3) / 4 +
                              "px;";
                            inline_video.parentNode.replaceChild(
                              elem,
                              inline_video
                            );
                          }
                    ...
                    if (mobile) {
                      ...
                      for (let elem of inline_images) {
                        ...
                        elem.removeAttribute("width");
                        elem.parentNode.removeAttribute("style");
                        elem.parentNode.parentNode.parentNode.removeAttribute(
                          "style"
                        );
                      }
                      let inline_data = document.querySelectorAll(
                        'div[data-layout="inline"][style]'
                      );
                      for (let elem of inline_data)
                        elem.removeAttribute("style");
                    }
                    let read_next = document.querySelector(
                      "div#cx-what-to-read-next"
                    );
                  ...
                  let cn_canonical = document.querySelector(
                    'head > link[rel="canonical"][href^="https://cn.wsj.com/"]'
                  );
                  if (!cn_canonical)
        ...
        let ads =
          "div.wsj-ad, div.adWrapper, div.css-xgokil-Box, div#cx-article-cover-overlay";
        hideDOMStyle(ads);
      } else if (matchDomain("zerohedge.com")) {
        window.setTimeout(function () {
          let paywall = document.querySelector(
            'div[class^="PremiumOverlay_container__"]'
          );
          if (paywall && dompurify_loaded) {
            removeDOMElement(paywall);
            let json_script = document.querySelector("script#__NEXT_DATA__");
            if (json_script) {
              try {
                ...
                if (json && json.props.pageProps.node.body) {
                  let article_new = parseHtmlEntities(
                    decode_utf8(
                      atob(json.props.pageProps.node.body.substring(21))
                    )
                  );
                  let article = document.querySelector(
                    'div[class^="NodeContent_mainContent__"'
                  );
                  if (article) {
                    article.innerHTML = "";
                    let parser = new DOMParser();
        ...
        matchDomain(ke_nation_media_domains) ||
        matchDomain(ke_nation_media_custom_domains)
      ) {
        let paywall = document.querySelectorAll(
          'div.modal, [id*="wall"], section.wall-guard'
        );
        if (paywall.length && dompurify_loaded) {
          removeDOMElement(...paywall);
          func_post = function () {
            let div_hidden = document.querySelectorAll(
              "div.article-page .nmgp"
            );
            for (let elem of div_hidden) elem.classList.remove("nmgp");
            let page_hidden = document.querySelector(
              "div.article-page .hidden"
            );
            if (page_hidden) page_hidden.classList.remove("hidden");
            let lazy_images = document.querySelectorAll(
              "img.lazy-img[data-srcset]:not([src])"
            );
            for (let elem of lazy_images) {
              elem.src = elem
        ...
        matchDomain(["oed.com"]) ||
        (window.location.hostname.replace(/^www\./, "").startsWith("oxford") &&
          document.querySelector(
            'div[id^="footer"] a[href="http://www.oup.com/"]'
          ))
      ) {
        ...
        header_nofix(
          article_sel,
          paywall_sel,
          cs_param.signin_text ||
            "BPC > Sign in with library card 12345678 or 11111111 & library of Royal Bor. of Kensington and Chelsea"
        );
      } else if (
        matchDomain(usa_gannett_domains) ||
        document.querySelector(
          'head > link[href*=".gannettdigital.com/"], head > link[href*=".gannett-cdn.com/"]'
        )
      ) {
        if (
          ...
          window.location.href = decodeURIComponent(
            window.location.href.split("?return=")[1]
          );
      } else if (
        matchDomain(usa_hearst_comm_domains) ||
        ...
        let ads = pageContains(
          "div > div > p",
          "Article continues below this ad"
        );
        for (let elem of ads) hideDOMElement(elem.parentNode.parentNode);
      } else if (
        (domain = matchDomain(usa_lee_ent_domains)) ||
        matchDomain(ca_torstar_domains.concat(["abqjournal.com"])) ||
        ...
        if (window.location.pathname.endsWith(".amp.html")) {
          ...
          for (let elem of elem_hidden) elem.removeAttribute("class");
          let amp_images = document.querySelectorAll(
            'div.main-content amp-img[src^="https://"]'
          );
          for (let amp_image of amp_images) {
            let elem = document.createElement("img");
            Object.assign(elem, {
          ...
          if (!domain) {
            ...
            removeDOMElement(paywall);
            let subscriber_only = document.querySelectorAll(
              "div.subscriber-only"
            );
            ...
            let banners = document.querySelectorAll(
              "div.subscription-required, div.redacted-overlay"
            );
            removeDOMElement(...banners);
          }
          let ads =
        ...
        matchDomain(usa_mcc_domains) ||
        document.querySelector(
          'section.bottom-nav > a[href^="https://www.mcclatchy.com/privacy-policy"]'
        )
      ) {
        if (window.location.hostname.startsWith("amp.")) {
          amp_unhide_subscr_section();
          ...
          let subscr_tag = document.querySelector(
            "div#subscriber-exclusive-tag"
          );
          let amp_players = document.querySelectorAll(
            "amp-connatix-player, amp-iframe.trinity-player"
          );
          removeDOMElement(subscr_tag, ...amp_players);
          let amp_images = document.querySelectorAll(
            "amp-img[srcset]:not([src])"
          );
          ...
          if (paywall && article) {
            func_post = function () {
              ...
              if (slideshow) {
                ...
                let image_wrappers =
                  slideshow.querySelectorAll("div.image-wrapper");
                for (let elem of image_wrappers) {
                  elem.removeAttribute("class");
                  elem.style = "margin: 20px 0px;";
                }
                let caption =
                  "div.mng-gallery-information-container, button.icon-close";
        ...
        let premium = document.querySelector(
          '.premium, div[class*="-premium"]'
        );
        if (premium) premium.removeAttribute("class");
      } else if (
        document.querySelector(
          'head > meta[property][content^="https://cdn.forumcomm.com/"]'
        )
      ) {
        ...
        let ads =
          'div.ad__section-border, div[id^="tbl_"], div.js-widget-content';
        hideDOMStyle(ads);
      } else csDone = true;
    }
  } // end csDone(Once)
...
if (
  ...
  let js_cookie = document.querySelector(
    "script#leaky_paywall_cookie_js-js-extra"
  );
...
function hideDOMElements(elements) {
  ...
  } else if (elements && elements.style) {
...
function header_nofix(header, cond_sel = "", msg = "BPC > no fix") {
  ...
  if (header && !document.querySelector("div#bpc_nofix")) {
    if (cond_sel) {
      ...
      if (elem.length) removeDOMElement(...elem);
      else return false;
    }
    let nofix_div = document.createElement("div");
    nofix_div.id = "bpc_nofix";
    nofix_div.style =
      "margin: 20px; font-size: 20px; font-weight: bold; color: red;";
...
function clearPaywall(paywall, paywall_action) {
  if (paywall) {
    if (!paywall_action) removeDOMElement(...paywall);
    else {
...
function getArchive(
  ...
  let url_archive =
    "https://" + archiveRandomDomain() + "/" + url.split(/[#\?]/)[0];
  let paywall = document.querySelectorAll(paywall_sel);
  if (paywall.length && dompurify_loaded) {
...
function getExtFetch(
  ...
  ext_api.runtime.sendMessage({
    ...
    data: {
      ...
      json_key: json_key,
      headers: headers,
      data_ext_fetch_id: data_ext_fetch_id,
    },
  });
}
  ...
  if (proxy) {
    ...
    if (!text_fail) {
      if (url.startsWith("https://archive."))
        text_fail =
          "BPC > Try for full article text (no need to report issue for external site):\r\n";
    ...
    ext_api.runtime.sendMessage({
      ...
      data: {
        ...
        selector_archive: selector_archive,
        base64: base64,
        text_fail: text_fail,
      },
    });
    ...
    window.setTimeout(function () {
      if (url.startsWith("https://archive.") && url_src) {
        ...
        html = html
          ...
          .replace(
            ...
            "$1" + "https://" + domain_archive + pathname
          );
      }
      let doc = parser.parseFromString(
      ...
      if (article_new) {
        if (article && article.parentNode) {
          if (url.startsWith("https://archive.")) {
            ...
            if (arch_dom) {
              ...
              arch_div.appendChild(
                archiveLink(
                  window.location.href,
                  "BPC > Full article text fetched from (no need to report issue for external site):\r\n"
                )
              );
            ...
            let invalid_links = article_new.querySelectorAll(
              'link[rel*="preload"]:not([href])'
            );
        ...
        replaceTextFail(
          ...
          article_link,
          proxy,
          text_fail.replace(":", no_content_msg)
        );
    }, 200);
  } else {
    replaceTextFail(
      ...
      article_link,
      proxy,
      url_src ? text_fail.replace(":", no_content_msg) : text_fail
    );
  }
}
...
function replaceTextFail(url, article, proxy, text_fail) {
  if (text_fail && article) {
    ...
    text_fail_div.setAttribute(
      "style",
      "margin: 0px 50px; font-weight: bold; color: red;"
    );
    text_fail_div.appendChild(document.createTextNode(text_fail));
    if (proxy) {
      if (url.startsWith("https://archive.")) {
        text_fail_div = archiveLink(
          url.replace(/^https:\/\/archive\.\w{2}\//, ""),
          text_fail
        );
      } else {
        let a_link = document.createElement("a");
    ...
    if (article.firstChild) article.firstChild.before(text_fail_div);
    else article.appendChild(text_fail_div);
  }
}
...
function amp_iframes_replace(weblink = false, source = "") {
  let amp_iframes = document.querySelectorAll(
  ...
  for (let amp_iframe of amp_iframes) {
    if (!weblink) {
      if (amp_iframe.offsetHeight > 10) {
        ...
        elem.style =
          "height: " +
          amp_iframe.offsetHeight +
          "px; width: 100%; border: 0px;";
...
function amp_redirect_not_loop(amphtml) {
  if (!check_loop()) {
    ...
    header_nofix(
      header,
      "",
      "BPC > redirect to amp failed (disable amp-to-html extension/add-on or browser setting)"
    );
  }
}
...
function amp_redirect(paywall_sel, paywall_action = "", amp_url = "") {
  let paywall = document.querySelectorAll(paywall_sel);
  let amphtml = document.querySelector('head > link[rel="amphtml"]');
  if (!amphtml && amp_url) amphtml = { href: amp_url };
...
function amp_unhide_subscr_section(
  amp_ads_sel = "",
  replace_iframes = true,
  amp_iframe_link = false,
  source = ""
) {
  ...
  removeDOMElement(...preview);
  let subscr_section = document.querySelectorAll(
...
function amp_unhide_access_hide(
  ...
  if (amp_access_not) {
    let amp_access_not_dom = document.querySelectorAll(
      "[amp-access" + amp_access_not + "]"
    );
...
function archiveLink(
  ...
  return externalLink(
    ["archive.today", archiveRandomDomain()],
    "https://{domain}?run=1&url={url}",
    url,
    text_fail
  );
}

function archiveLink_renew(
  url,
  text_fail = "BPC > Only use to renew if text is incomplete or updated:\r\n"
) {
  return externalLink(
    [new URL(url).hostname],
    "{url}/again?url=" + window.location.href,
    url,
    text_fail
  );
}

function googleSearchToolLink(
  url,
  text_fail = "BPC > Try for full article text (test url & copy html (tab) code to [https://codebeautify.org/htmlviewer]):\r\n"
) {
  return externalLink(
    ["search.google.com"],
...
function readMediumLink(
  url,
  text_fail = "BPC > Try for full article text:\r\n"
) {
  return externalLink(
    ["readmedium.com"],
    "https://{domain}/{url}",
...
function externalLink(
  ...
  ext_url_templ,
  url,
  text_fail = "BPC > Full article text:\r\n"
) {
  let text_fail_div = document.createElement("div");
...
function removeClassesByPrefix(el, prefix) {
  ...
  for (let el_class of el_classes) {
    if (el_class.startsWith(prefix)) el_classes.remove(el_class);
  }
}
...
function insert_script(func, insertAfterDom) {
  ...
  if (!bpc_script) {
    ...
    let insertAfter = insertAfterDom
      ? insertAfterDom
...
function getSourceJsonScript(filter, attributes = ":not([src], [type])") {
  ...
  for (let script of scripts) {
    if (script.text.match(filter)) return script;
  }
  return false;
}
    ...
    try {
      ...
      for (let par of pars) {
        ...
        for (let elem of story_elements) {
            ...
            if (elem["image-s3-key"]) {
              ...
              if (elem.title) {
                ...
                if (elem.title.includes("</")) {
                  let doc = parser.parseFromString(
                    "<div>" +
                      DOMPurify.sanitize(elem.title, dompurify_options) +
                      "</div>",
                    "text/html"
                  );
          ...
          } else if (!["widget"].includes(elem.type)) console.log(elem);
  ...
  if (Array.isArray(json)) {
    ...
    for (let elem in json) {
      ...
      if (typeof json_elem === "string" && matchKeyJson(elem, keys)) {
        ...
        Array.isArray(json_elem) &&
        json_elem.length > 1 &&
        matchKeyJson(elem, keys)
      ) {
        return json_elem;
      } else source = source || findKeyJson(json_elem, keys, min_val_len);
    }
  }
  return source;
}
...
function getJsonUrlText(
  ...
  if (url_rest) json_url = json_url.replace("/wp-json/", "/?rest_route=/");
  else if (url_slash) json_url = json_url.replace("/wp-json/", "//wp-json/");
  if (json_url) {
    fetch(json_url).then((response) => {
      if (response.ok) {
        response.json().then((json) => {
          try {
            let json_text = parseHtmlEntities(
              !key ? json.content.rendered : getNestedKeys(json, key)
            );
...
function randomIP(range_low = 0, range_high = 223) {
  let rndmIP = [];
  for (let n = 0; n < 4; n++) {
    if (n === 0) rndmIP.push(range_low + randomInt(range_high - range_low + 1));
    else rndmIP.push(randomInt(255) + 1);
  }
  return rndmIP.join(".");
}
...
function parseHtmlEntities(encodedString) {
  let parser = new DOMParser();
  let doc = parser.parseFromString(
    "<textarea>" + encodedString + "</textarea>",
    "text/html"
  );
